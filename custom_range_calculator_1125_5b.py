...
def process_cluster_tables_two_pass(
    ...
    Returns:
        prep_df (DataFrame), processing_summary (dict)
    """
    # Normalize valid lists to sets
    if valid_list_pass1 is None:
        valid_list_pass1 = set()
    elif not isinstance(valid_list_pass1, set):
        valid_list_pass1 = set(valid_list_pass1)

    if valid_list_pass2 is None:
        valid_list_pass2 = set()
    elif not isinstance(valid_list_pass2, set):
        valid_list_pass2 = set(valid_list_pass2)
    ...
    try:
        # Basic safety: no measurement file
        if measurement_df is None or measurement_df.empty:
            st.warning("⚠️ Measurement file is empty or missing.")
            return pd.DataFrame(), empty_summary

        # -----------------------------
        # Diagnostic: M # availability
        # -----------------------------
        m_num_col = next(
            (c for c in ['M #', 'm #', 'M Number', 'm number']
        ...
        if m_num_col:
            ...
            # Only show overlaps if a valid list was actually provided
            if valid_list_pass1:
                st.info(
                    "✓ Pass 1 M #s available: "
                    f"{sorted(pass1_overlap) if pass1_overlap else 'None'}"
                )
            if valid_list_pass2:
                # Show first 10 so it doesn't get crazy long
                if pass2_overlap:
                    sorted_p2 = sorted(pass2_overlap)
                    preview = sorted_p2[:10]
                    extra = " ..." if len(sorted_p2) > 10 else ""
                    st.info(
                        "✓ Pass 2 M #s available: "
                        f"{preview}{extra}"
                    )
                else:
                    st.info("✓ Pass 2 M #s available: None")
        else:
            st.error("❌ No 'M #' column found in measurement file!")
            return pd.DataFrame(), empty_summary

        # -----------------------------------------
        # Pre-filter measurement_df for each pass
        # -----------------------------------------
        measurement_df_pass1 = filter_measurement_by_m_numbers(
            measurement_df, valid_list_pass1
        )
        measurement_df_pass2 = filter_measurement_by_m_numbers(
            measurement_df, valid_list_pass2
        )

        # Collect all origins from both feeds (for consistency / potential debugging)
        hlc_df_list = [(small_df, 'Small')]
        if big_df is not None:
            hlc_df_list.append((big_df, 'Big'))
        _ = _collect_origins(hlc_df_list)  # kept for future use / debugging
        ...
        for hlc_df, feed_name in hlc_df_list:
            ...
            # Get origins present in THIS feed
            feed_origins = []
            for col in hlc_df.columns:
                if col.endswith(" H"):
                    origin = col[:-2]
            ...
            for origin_name in feed_origins:
                # Collect HLC changes for this origin & feed
                hlc_data = find_new_data_changes(
                    hlc_df,
                    report_time,
                    origin_name,
                    scope_days
                ...
                # Sort by datetime DESC so most-recent first
                hlc_data_sorted = sorted(
                    hlc_data,
                ...
                # ---------------------------
                # PASS 1: first 2 most recent days
                # ---------------------------
                if hlc_data_sorted is not None \
                        and not (measurement_df_pass1 is None
                                 or measurement_df_pass1.empty):
                    unique_dates = []
                    ...
                    if unique_dates:
                        pass1_days = set(unique_dates[:2])
                        pass1_data = [
                            item for item in hlc_data_sorted
                            if item['datetime'].date() in pass1_days
                        ]
                        ...
                        for hlc_item in pass1_data:
                            ...
                            # ✅ Compute raw M range from price range
                            calc = calculate_raw_m_values(
                                hlc_item,
                                range_low,
                                range_high
                            )
                            ...
                            # Enrich HLC dict with avg/spread/raw_m bounds
                            hlc_enriched = {**hlc_item, **calc}

                            valid_results = find_valid_m_values(
                                measurement_df_pass1,
                                raw_m_low=calc['raw_m_low'],
                                raw_m_high=calc['raw_m_high'],
                                hlc_data=hlc_enriched,
                                range_low=range_low,
                ...
                # ---------------------------
                # PASS 2: all data in scope
                # ---------------------------
                if hlc_data_sorted is not None \
                        and not (measurement_df_pass2 is None
                                 or measurement_df_pass2.empty):
        ...
        processing_summary = {
            ...
            'pass1_m_numbers': sorted(
                set(
                    int(float(r['M #']))
                    for r in all_pass1_results
                    if r.get('M #') not in (None, "")
                )
            ) if all_pass1_results else [],
            'pass2_m_numbers': sorted(
                set(
                    int(float(r['M #']))
                    for r in all_pass2_results
                    if r.get('M #') not in (None, "")
                )
            ) if all_pass2_results else [],
            'pass1_origins': sorted(
                set(
                    r['Origin']
                    for r in all_pass1_results
                    if r.get('Origin')
        ...
        if not combined_results:
            # Still return the summary so you can see the diagnostics in Tab 8
        ...
        if not prep_df.empty:
            ...
            prep_df = prep_df.drop_duplicates(
                subset=['M #', 'Origin', 'Arrival'],
                keep='first'
            )
            prep_df = prep_df.sort_values(
                ['Arrival', 'Output'],
