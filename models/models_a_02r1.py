    ...
    for i in range(len(rows)):
        ...
        for j in range(i, len(rows)):
            ...
            if m in {0, 40, -40, 54, -54}:
                if len(path) >= 1:  # Allow sequences with at least 2 points (1 prior + ending)
                    path.append(j)
                    raw_sequences.append(rows.loc[path])
                break
                
            # Continue building sequence if descending
...
def detect_A_models(df):
    """
    Main detection function - simplified to avoid overly restrictive filtering
    """
    ...
    for output in df["Output"].unique():
        ...
        for seq in full_matches:
            ...
            # SIMPLIFIED LOGIC - Remove overly restrictive valid_endings filtering
            # Just use the last row as the final traveler and all prior rows as context
            last = seq.iloc[-1]
            prior = seq.iloc[:-1]
