# Fixed Model C with corrected C.02 opposites and C.04 ascending detection
# All issues addressed: enhanced time matching and sequence detection
...
# FIXED C.02: Enhanced opposites detection with better time matching
def classify_c02_sequence(seq):
    ...
    # FIXED: Enhanced time classification with broader matching
    arrival_time = pd.to_datetime(seq.iloc[-1]["Arrival"])
    hour = arrival_time.hour
    minute = arrival_time.minute
    
    # More flexible time matching - FIXED the logic
    if hour == 17 or (hour == 18 and minute == 0):  # 17:00-18:00 range
        suffix = "[O0]"; num = "3"
    elif hour >= 18 and hour <= 23:  # Evening hours (18:01-23:59)
        suffix = "[E0]"; num = "2" 
    elif hour >= 0 and hour < 17:  # Late hours (00:00-16:59)
        suffix = "[L0]"; num = "1"
    else:
        return None, None
...
# ENHANCED: Find all sequence types for better detection
def find_descending_sequences(df):
    """Find descending sequences AND ascending/opposites patterns"""
    sequences = []
    seen_signatures = set()
    ...
    for output in df["Output"].unique():
        ...
        # ADDED: Find 3-point patterns (opposites and ascending)
...
def detect_C_models(df, run_c01=True, run_c02=True, run_c04=True):
    sequences = find_descending_sequences(df)  # Use enhanced detection
