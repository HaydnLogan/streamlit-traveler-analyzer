...
def detect_A_models(df):
    """
    Main detection function - finds ALL valid sequences with different endings
    """
    ...
    for output in df["Output"].unique():
        ...
        for seq in full_matches:
            # Basic validation - must have at least 2 rows and end with valid M#
            if seq.shape[0] < 2 or abs(seq.iloc[-1]["M #"]) not in {0, 40, 54}:
                continue
                
            sig = sequence_signature(seq)
            # Allow multiple sequences with same signature but different origins
            sig_with_origins = (sig, tuple(seq["Origin"].tolist()))
            if sig_with_origins in all_signatures:
                continue
            all_signatures.add(sig_with_origins)
            
            # Use the last row as the final traveler and all prior rows as context
            last = seq.iloc[-1]
            prior = seq.iloc[:-1]
        ...
        # Find 2-member pairs not already used
        pairs = find_pairs(subset, {sig for sig, _ in all_signatures})
        for seq in pairs:
            sig = sequence_signature(seq)
            sig_with_origins = (sig, tuple(seq["Origin"].tolist()))
            if sig_with_origins in all_signatures:
...
def show_a_model_results(model_outputs, report_time):
    ...
    for code, base_label in base_labels.items():
        for suffix, prefix in [("", "2+ to"), ("pr", "Pair to")]:
            key = code + suffix
