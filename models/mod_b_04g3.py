...
# Cluster Table above.  GroundTech revision to Descending sequences to catch missing b02a[]0
...
def find_descending_sequences(df, min_length=3, max_window=6):
    sequences = []
    seen_signatures = set()
    ...
    for output in df["Output"].unique():
        ...
        # Generate all combinations of row indices within window size
        for combo in combinations(indices, r=min_length):
            if combo[-1] - combo[0] > max_window:
                continue  # skip if spread too far apart

            seq = rows.loc[list(combo)].reset_index(drop=True)

            m_vals = seq["M #"].tolist()
            abs_vals = [abs(m) for m in m_vals]

            if len(set(abs_vals)) != len(abs_vals):
                continue  # no repeated abs(M #)

            if abs_vals != sorted(abs_vals, reverse=True):
                continue  # must be strictly descending by abs
...
def show_b40_today_cluster(model_outputs):
    ...
    if cluster:
        st.subheader("🌐 B Model |40| Today — Cluster Report Table")
