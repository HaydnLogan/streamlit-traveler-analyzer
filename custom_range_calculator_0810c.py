...
def get_wasp_macedonia_data(hlc_df, report_time, origin_name, start_hour=18):
    ...
    These origins have dedicated columns and follow weekly/monthly schedules.
    
    Args:
        hlc_df: DataFrame containing the H/L/C columns
        report_time: Report datetime
        origin_name: Origin name (e.g., 'WASP-12b', 'WASP-12b[1]', 'Macedonia', 'Macedonia[2]')
        start_hour: Start hour (17 or 18)
    
    Returns:
        Dictionary with H, L, C, datetime, and origin, or None if not found
    """
    try:
        ...
        if not all(col in hlc_df.columns for col in [h_col, l_col, c_col]):
            st.warning(f"Missing columns for {origin_name}: {h_col}, {l_col}, {c_col}")
            return None
        
        # Get the most recent row with valid H/L/C data
        # Convert time column for filtering
        hlc_df_copy = hlc_df.copy()
        if 'time' in hlc_df_copy.columns:
            hlc_df_copy['time_clean'] = pd.to_datetime(hlc_df_copy['time'], errors='coerce')
        else:
            hlc_df_copy['time_clean'] = pd.to_datetime(hlc_df_copy.iloc[:, 0], errors='coerce')
        
        # Filter to data at or before report time
