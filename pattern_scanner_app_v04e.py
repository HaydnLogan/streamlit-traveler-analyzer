...
st.set_page_config(
    page_title="pattern scanner v04e",
    page_icon="🎯",
    layout="wide",
    ...
    try:
        ...
        for feed_name, df in ohlc_feeds.items():
            try:
                ...
                # Parse time as-is and drop timezone WITHOUT conversion
                df_copy['time'] = pd.to_datetime(df_copy['time'])
                if hasattr(df_copy['time'].dt, 'tz') and df_copy['time'].dt.tz is not None:
    ...
    if not traveler_file:
        ...
        with st.expander("📖 Quick Start Guide", expanded=True):
            ...
            **Use Nested Swing Detector to identify major turns in the trading day, then determine WHY those turns and exits happened based on what Travelers were present at or before the turn times.**
            ...
            5. **Track Movement**: Monitor how price moved after each zone (turn-to-turn distances)
            ...
            try:
                ...
                if big_feed_file:
                    ...
                    if 'time' in big_df.columns:
                        ...
                        if hasattr(big_df['time'].dt, 'tz') and big_df['time'].dt.tz is not None:
                ...
                if small_feed_file:
                    ...
                    if 'time' in small_df.columns:
                        ...
                        if hasattr(small_df['time'].dt, 'tz') and small_df['time'].dt.tz is not None:
...
def add_lookforward_analysis(zones_df: pd.DataFrame,
    ...
    # Prepare OHLC data - drop timezone WITHOUT conversion
    ohlc_df = ohlc_df.copy()
    ohlc_df['time'] = pd.to_datetime(ohlc_df['time'])
    if hasattr(ohlc_df['time'].dt, 'tz') and ohlc_df['time'].dt.tz is not None:
