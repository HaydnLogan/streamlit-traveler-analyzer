...
def main():
    st.title(" Market Swing Analysis Tool 05e_22b Two Pass")
            ...
            try:
                ...
                if st.button("🚀 Process All Models", key="gen_all_models", type="primary"):
                    if not bypass_mode_active:
                        ...
                        # BYPASS MODE - Use uploaded traveler report directly with OHLC
                        with st.spinner("Processing all 23 trading models using uploaded traveler report..."):
                            ...
                            # In bypass mode, the uploaded report IS the cluster table
                            # We treat it as if it came from one feed
                            # The model processor expects small_df/big_df format, so we use the report as small_df
                            
                            # Note: In bypass mode, we don't have measurement_df, but the traveler 
                            # report should already have the necessary M# relationships embedded
                            
                            # Create a minimal measurement_df if needed by some models
                            # (Most models work directly with cluster table data)
                            bypass_measurement_df = pd.DataFrame()
                            
                            all_results = process_all_models(
                                measurement_df=bypass_measurement_df,  # Empty or minimal
                                small_df=small_hlc_df,  # This is the uploaded traveler report
                                big_df=None,  # No big feed in bypass mode
                                report_time=report_time,
                                ...
                                combine_segments=True,
                                feed_selection="Small feed only",  # Only have one "feed" (the report)
