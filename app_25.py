...
st.header("🧬 Data Feed Processor + Model A/B/C Detector. v25")
...
if small_feed_file and big_feed_file and measurement_file:
    try:
        ...
        if input_value_18 is None:
            ...
            results = []
            # Generate raw data (no filtering during generation)
            ...
            if len(final_df) > 0:
                ...
                st.subheader("📊 Final Traveler Report (Raw Dataset)")
                
               # Show raw data without highlighting for performance
                st.dataframe(display_df)

                # Now apply traveler report filtering as separate reports below
                if use_full_range or use_custom_ranges:
                    st.markdown("---")
                    st.subheader("🎯 Filtered Traveler Reports")
                    
                    # Apply the proven filtering logic from the earlier part of app.py
                    if use_full_range:
                        # Full Range filtering
                        possible_cols = [f"Input @ {day_start_hour:02d}:00", "Input @ 18:00", "Input @ 17:00"]
                        input_ref_col = None
                        for col in possible_cols:
                            if col in final_df.columns:
                                input_ref_col = col
                        ...
                        if input_ref_col:
                            input_ref_value = final_df[input_ref_col].iloc[0]
                            upper_bound = input_ref_value + full_range_value
                            lower_bound = input_ref_value - full_range_value
                            st.info(f"📏 Full Range Filter: {lower_bound:.1f} to {upper_bound:.1f}")
                            ...
                            if len(range_df) > 0:
                                ...
                                # Include Range and Zone in column order
                                range_columns = base_columns + ["Range", "Zone"]
                                display_columns = [col for col in range_columns if col in display_range_df.columns]
                                display_range_df = display_range_df[display_columns]
                                
                                st.subheader("📊 Full Range Filtered Report")
                                styled_range_df = highlight_traveler_report(display_range_df)
                                st.dataframe(styled_range_df)
                                ...
                                with ExcelWriter(excel_buffer_full, engine="xlsxwriter") as writer:
                                    styled_excel = highlight_traveler_report(display_range_df)
                                    styled_excel.to_excel(writer, index=False, sheet_name="Full Range Report")
                                
                                st.download_button(
                                    "📥 Download Full Range Report (Excel)",
                                    data=excel_buffer_full.getvalue(),
                                    file_name=f"full_range_report_{timestamp_str}.xlsx",
                                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                                )
                            else:
                                st.warning("No data found within the Full Range.")
                    ...
                    if use_custom_ranges:
                        ...
                        for range_info in custom_ranges:
                            ...
                            if len(range_df) > 0:
                                ...
                                # Calculate zones
                                if range_info["type"] == "high":
                                    distance = range_info["upper"] - range_df["Output"]
                                else:
                                    distance = range_df["Output"] - range_info["lower"]
                                
                                range_df["Zone"] = distance.apply(lambda d: 
                                    "0 to 6" if d <= 6 else
                                    "6 to 12" if d <= 12 else
                                    "12 to 18" if d <= 18 else
                                    "18 to 24"
                                )
                        ...
                        if all_custom_data:
                            combined_custom_df = pd.concat(all_custom_data, ignore_index=True)
                            combined_custom_df.sort_values(by=["Output", "Arrival"], ascending=[False, True], inplace=True)
                            
                            # Format display
                            display_custom_df = combined_custom_df.copy()
                            display_custom_df["Arrival"] = display_custom_df["Arrival"].dt.strftime("%a %y-%m-%d %H:%M")
                            
                            # Include Range and Zone in column order
                            custom_columns = base_columns + ["Range", "Zone"]
                            display_columns = [col for col in custom_columns if col in display_custom_df.columns]
                            display_custom_df = display_custom_df[display_columns]
                            
                            st.subheader("📊 Custom Ranges Filtered Report")
                            styled_custom_df = highlight_custom_traveler_report(display_custom_df, show_highlighting=True)
                            st.dataframe(styled_custom_df)
                            
                            # Excel download
                            timestamp_str = report_time.strftime("%y-%m-%d_%H-%M") if report_time else "unknown"
                            excel_buffer_custom = io.BytesIO()
                            with ExcelWriter(excel_buffer_custom, engine="xlsxwriter") as writer:
                                styled_excel = highlight_custom_traveler_report(display_custom_df, show_highlighting=True)
                                styled_excel.to_excel(writer, index=False, sheet_name="Custom Ranges Report")
                            
                            st.download_button(
                                "📥 Download Custom Ranges Report (Excel)",
                                data=excel_buffer_custom.getvalue(),
                                file_name=f"custom_ranges_report_{timestamp_str}.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                            )
                        else:
                            st.warning("No data found within any of the Custom Ranges.")
            else:
            ...
            with ExcelWriter(excel_buffer, engine="xlsxwriter") as writer:
                # Apply highlighting only for Excel export
