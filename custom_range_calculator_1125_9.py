    ...
    try:
        ...
        for hlc_df, feed_name in hlc_df_list:
            ...
            if allowed_origins is not None:
                ...
                if not feed_origins:
                    st.info(f"ℹ️ {feed_name} Feed: No matching origins after filtering")
                    continue
                st.info(f"✓ {feed_name} Feed: Processing {len(feed_origins)} filtered origins: {', '.join(feed_origins)}")
