...
st.set_page_config(page_title="Market Swing Analysis 03f", layout="wide")
st.header("📈 Market Swing Analysis Tool 03f")
...
def find_best_from_point_for_hod_lod(hod_lod_time, hod_lod_price, day_data, all_swings):
    """Find the best reference point for HOD/LOD swing calculation"""
    # Convert to pandas datetime if needed
    if isinstance(hod_lod_time, str):
        hod_lod_time = pd.to_datetime(hod_lod_time)
    
    # Get all points (swing endpoints and OHLC data) that occurred before HOD/LOD
    candidates = []
    
    # Add swing endpoints as candidates
    for swing in all_swings:
        swing_time = swing['to_time']
        if swing_time < hod_lod_time:
            candidates.append({
                'time': swing_time,
                'price': swing['to_price'],
                'type': 'swing_end',
                'priority': 1  # Prefer swing endpoints
            })
    ...
    # Sort by time (most recent first) then by priority
    candidates.sort(key=lambda x: (-x['time'].timestamp(), x['priority']))
    
    # Return the most recent, highest priority candidate
...
if uploaded_file is not None:
        ...
        if st.button("🚀 Run Analysis"):
            ...
            if daily_stats:
                ...
                for day_stat in daily_stats:
                    ...
                    if not hod_covered:
                        from_point = find_best_from_point_for_hod_lod(daily_high_time, daily_high, day_data, all_swings)
                    ...
                    if not lod_covered:
                        from_point = find_best_from_point_for_hod_lod(daily_low_time, daily_low, day_data, all_swings)
    ...
    **Key Features:**
    ...
    - 🔧 **NEW**: HOD/LOD entries now have proper swing information calculated!
    """)
