...
st.set_page_config(page_title="Market Swing Analysis 03h", layout="wide")
st.header("📈 Market Swing Analysis Tool 03h")
...
def find_best_from_point_for_hod_lod(hod_lod_time, hod_lod_price, day_data, all_swings, is_hod=True):
    """Find the best reference point for HOD/LOD swing calculation - FIXED VERSION"""
    # Convert to pandas datetime if needed
    if isinstance(hod_lod_time, str):
        hod_lod_time = pd.to_datetime(hod_lod_time)
    
    # Get all points that occurred BEFORE HOD/LOD (strict inequality)
    candidates = []
    
    # Add swing endpoints as candidates (highest priority)
    for swing in all_swings:
        swing_time = swing['to_time']
        if swing_time < hod_lod_time:  # Must be strictly before
            candidates.append({
                'time': swing_time,
    ...
    # Add OHLC data as candidates (lower priority)
    for idx, row in day_data.iterrows():
        if row['time'] < hod_lod_time:  # Must be strictly before (no same-bar allowed)
            # Add OHLC points as candidates
    ...
    # If no candidates found (e.g., HOD/LOD at session start), use session start as fallback
    if not candidates:
        session_start_row = day_data.iloc[0]
        # For HOD at session start, use the low of the first bar or open
        # For LOD at session start, use the high of the first bar or open
        if is_hod:
            # HOD: measure from session start low or open (whichever is lower)
            reference_price = min(session_start_row['low'], session_start_row['open'])
        else:
            # LOD: measure from session start high or open (whichever is higher)
            reference_price = max(session_start_row['high'], session_start_row['open'])
        
        return {
            'time': session_start_row['time'],
            'price': reference_price
        }
    
    # Sort by time (most recent first) then by priority (lower number = higher priority)
    candidates.sort(key=lambda x: (-x['time'].timestamp(), x['priority']))
    
    # For HOD: prefer the most recent low point or swing low
    # For LOD: prefer the most recent high point or swing high
    best_candidate = None
    
    if is_hod:
        # For HOD, look for recent lows first
        for candidate in candidates:
            if candidate['type'] in ['low', 'swing_end']:
                # Check if this swing ended at a low
                if candidate['type'] == 'swing_end':
                    # Find the corresponding swing to check if it ended at a low
                    corresponding_swing = None
                    for swing in all_swings:
                        if swing['to_time'] == candidate['time'] and swing['to_price'] == candidate['price']:
                            corresponding_swing = swing
                            break
                    if corresponding_swing and corresponding_swing['type'] == 'low':
        ...
        # For LOD, look for recent highs first  
        for candidate in candidates:
            if candidate['type'] in ['high', 'swing_end']:
                # Check if this swing ended at a high
                if candidate['type'] == 'swing_end':
                    # Find the corresponding swing to check if it ended at a high
                    corresponding_swing = None
                    for swing in all_swings:
                        if swing['to_time'] == candidate['time'] and swing['to_price'] == candidate['price']:
                            corresponding_swing = swing
                            break
                    if corresponding_swing and corresponding_swing['type'] == 'high':
                        best_candidate = candidate
                        break
                else:
                    best_candidate = candidate
    ...
    # If no ideal candidate found, just use the most recent, highest priority candidate
    ...
    try:
        ...
        if st.button("🚀 Run Analysis"):
            ...
            if daily_stats:
                ...
                # Create detailed NY swings DataFrame - NY swings AND standalone HOD/LOD WITH PROPER SWING INFO
                detailed_ny_swings = []
                for day_stat in daily_stats:
                    ...
                    # Get day data for HOD/LOD calculations
                    day_data = analysis_df[analysis_df['time'].apply(lambda x: get_trading_day(x, day_start_hour)) == trading_day]
                    day_data = day_data.sort_values('time')
                    
                    # Add NY swings with HOD/LOD connections
                    ny_swings = day_stat.get('ny_swings', [])
                    all_swings = day_stat.get('all_swings', [])
                    hod_covered = False
                    lod_covered = False
                    ...
                    # FIXED: Add standalone HOD entry WITH CALCULATED SWING INFO if not covered by NY swings
                    if not hod_covered:
                        from_point = find_best_from_point_for_hod_lod(daily_high_time, daily_high, day_data, all_swings, is_hod=True)
                        
                        if from_point and from_point['price'] != daily_high and from_point['time'] != daily_high_time:
                            move_size = abs(daily_high - from_point['price'])
                            direction = 'up' if daily_high > from_point['price'] else 'down'
                            ...
                            detailed_ny_swings.append({
                                ...
                                'swing_type': 'high',
                                'direction': direction,
                                'from_datetime': from_point['time'].strftime('%Y-%m-%d %H:%M'),
                                'from_price': from_point['price'],
                            ...
                            # Ultimate fallback: Use session open if no better point found
                            session_start_row = day_data.iloc[0]
                            session_open = session_start_row['open']
                            move_size = abs(daily_high - session_open)
                            direction = 'up' if daily_high > session_open else 'down'
                            category = categorize_swing(move_size)
                    ...
                    # FIXED: Add standalone LOD entry WITH CALCULATED SWING INFO if not covered by NY swings
                    if not lod_covered:
                        from_point = find_best_from_point_for_hod_lod(daily_low_time, daily_low, day_data, all_swings, is_hod=False)
                        
                        if from_point and from_point['price'] != daily_low and from_point['time'] != daily_low_time:
                            move_size = abs(from_point['price'] - daily_low)
                            direction = 'down' if daily_low < from_point['price'] else 'up'
                            category = categorize_swing(move_size)
                ...
                # Prepare export data with proper datetime handling
                ...
                # Build detailed swings for export with proper datetime objects
                ...
                for day_stat in daily_stats:
                    for s in day_stat.get('all_swings', []):
                        detailed_swings_export.append({
                            ...
                            'from_datetime': s['from_time'],   # datetime object
                            'from_price': s['from_price'],
                            'to_datetime': s['to_time'],       # datetime object
                            'to_price': s['to_price'],
                            'move_size': s['move_size'],
                            'category': s['category']
                        })
                
                detailed_df_export = pd.DataFrame(detailed_swings_export) if detailed_swings_export else pd.DataFrame()
                
                # NY detailed swings for export - WITH PROPER SWING INFO
                detailed_ny_export = []
                for record in detailed_ny_swings:
                    export_record = record.copy()
                    # Convert string datetimes back to datetime objects for export (unless it's 'n/a')
                ...
                detailed_ny_df_export = pd.DataFrame(detailed_ny_export) if detailed_ny_export else pd.DataFrame()
                ...
                # FIXED: Download buttons with unique keys
