"""
1125 update to version 0813.  Add first and second pass for Swing Analysys Tool. 
...
def process_cluster_tables_two_pass(
    ...
    try:
        ...
        origins = _collect_origins([small_df, big_df] if big_df is not None else [small_df])
        
        all_pass1_results = []
        all_pass2_results = []
        
        for origin_name in origins:
            # Get HLC data changes
            hlc_data = find_new_data_changes(small_df, report_time, origin_name, scope_days)
                ...
                if len(unique_dates) > 0:
                    ...
                    for hlc_item in pass1_data:
                        ...
                        valid_results = find_valid_m_values(
                            ...
                            is_high_range=False,
                            data_source=f"Pass1_{origin_name}",
                            report_time=report_time
                        )
                        
                        # Filter by valid list pass 1
                        if valid_results:
                            for result in valid_results:
                                if result['M #'] in valid_list_pass1:
            ...
            for hlc_item in hlc_data_sorted:
                ...
                valid_results = find_valid_m_values(
                    ...
                    range_high=range_high,
                    is_high_range=False,
                    data_source=f"Pass2_{origin_name}",
                    report_time=report_time
                )
                
                # Filter by valid list pass 2
                if valid_results:
                    for result in valid_results:
                        if result['M #'] in valid_list_pass2:
