...
st.header("🧬 Traveler Report Generator v34 | Nested Swing Detector + Look Back Days")
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 🌟 Origin Filtering")
enable_origin_filtering = st.sidebar.checkbox(
    "Enable Origin Filtering",
    value=False,
    help="Filter travelers by celestial body origins"
)

allowed_origins = None
if enable_origin_filtering:
    st.sidebar.markdown("**Select Origin Groups:**")
    use_epic = st.sidebar.checkbox("Epic Origins", value=True, help="Include Epic origins (Spain, Saturn, Jupiter)")
    use_anchor = st.sidebar.checkbox("Anchor Origins", value=False, help="Include Anchor origins (Kepler-62, Kepler-44, Trinidad, Tobago)")
    use_other = st.sidebar.checkbox("Other Origins", value=False, help="Include WASP, Macedonia, etc.")
    
    allowed_origins = []
    if use_epic:
        allowed_origins.extend(EPIC_ORIGINS)
    if use_anchor:
        allowed_origins.extend(ANCHOR_ORIGINS)
    if use_other:
        # Add WASP and Macedonia patterns
        allowed_origins.extend(['WASP', 'Macedonia'])
    
    if allowed_origins:
        st.sidebar.info(f"Filtering for {len(allowed_origins)} origin groups")
    else:
        st.sidebar.warning("⚠️ No origins selected - will show all data")
        allowed_origins = None
            ...
            if file:
                try:
                    ...
                    if allowed_origins is not None:
                        # Base columns that must always be kept
                        base_cols = ['time', 'open', 'high', 'low', 'close', 'volume', 'Name']
                        
                        # Find all origin columns matching allowed_origins
                        origin_cols = []
                        for col in df.columns:
                            if col not in base_cols:
                                # Check if column matches any allowed origin
                                col_lower = col.lower()
                                for origin in allowed_origins:
                                    origin_lower = origin.lower()
                                    if origin_lower in col_lower:
...
st.markdown("---")
st.caption("🌌 Traveler Report Generator v34 | Nested Swing Detector + Look Back Days + Fixed Input @ start bug + Updated naming")
