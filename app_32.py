# v32 - Traveler App - AWS Accelerated
...
asset_id = st.selectbox("Asset", ["NQ", "ES", "YM", "RTY"])
high1 = st.number_input("High Range 1", value=0.0)
...
if st.button("Run Query"):
    payload = {
        ...
        "custom_ranges": {
            "High 1": {"enabled": high1 > 0, "value": high1}
        },
    ...
    if response.status_code == 200:
        data = response.json()
        st.success(f"Found {data['count']} entries")
