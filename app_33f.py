...
# Changes from v33e:
# - Fixed Arrival datetime formatting: All cells now proper Excel datetimes displaying as 12/04/2025 18:00
# - Fixed single mode highlights: Restored xlsxwriter for single-day exports
# - Added HOD/LOD zone columns: 'sm HOD/LOD' and 'bg HOD/LOD' with color-coded zones (0-9, 09-18, 18-27)
# - HOD zones: Red (#FF5050), Light Red (#FF9999), Orange (#FCD5B4)
# - LOD zones: Blue (#00B0F0), Sky Blue (#66FFFF), Orange (#FCD5B4)

# v33e - 12.27.25 FIXED grab times, added timer, integrated highlighting
# Changes from v33d:
# - Fixed grab times: Now starts with previous day's 1800 (e.g., select 11-Dec → gets 10Dec 1800, 11Dec 0315/0900/1230)
# - Added processing timer that reports total time to generate multi-day reports
# - Integrated apply_excel_highlighting for multi-day exports (using xlsxwriter)
# - Multi-day Excel exports now have colored highlighting like single-day reports
...
def create_multi_day_excel(all_day_reports, asset_id, trading_day_base):
    ...
    # Use xlsxwriter for highlighting support
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        workbook = writer.book
        
        for tab_name, df in all_day_reports:
            # Write dataframe to sheet
            df.to_excel(writer, sheet_name=tab_name, index=False, startrow=0)
            ...
            # Apply highlighting with HOD/LOD support
            try:
...
def render_unified_export(traveler_reports, report_time, asset_id=""):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        for tab_name, df_data in traveler_reports.items():
            ...
            # Coerce arrival datetime
            df_coerced = _coerce_arrival_datetime(df_data)
            
            # Write to Excel
            df_coerced.to_excel(writer, sheet_name=tab_name, index=False, startrow=0)
            ...
            # Auto-filter on header row
            if len(df_coerced) > 0:
                worksheet.autofilter(0, 0, len(df_coerced), len(df_coerced.columns) - 1)
            
            # Apply highlighting with HOD/LOD support
            try:
                apply_excel_highlighting_with_hodlod(workbook, worksheet, df_coerced, is_custom_ranges=False)
            except Exception as e:
                st.warning(f"Highlighting skipped for '{tab_name}': {e}")
...
# Streamlit app
st.title("🌌 Traveler Report Generator v33f")
...
if st.button("🚀 Process Data"):
    ...
    if bypass_df is not None:
        ...
        if use_full_range_single:
            ...
            if final_df_filtered is None or final_df_filtered.empty:
                ...
                if report_time.hour == 18:
                    ...
                    trading_day_start = report_time.replace(hour=18, minute=0, second=0)
                    if report_time.hour < 18:
                        trading_day_start = trading_day_start - dt.timedelta(days=1)
        ...
        # ---------- 2) FULL RANGE (MULTIPLE) MODE ----------
            ...
            # Note: First grab (1800) is on the PREVIOUS day (e.g., 10Dec 1800 for 11Dec trading day)
            ...
            for day_offset in range(num_days_multi):
                ...
                for idx, (hour, minute) in enumerate(grab_times):
                    # First grab (1800) uses previous day, rest use current day
                    if idx == 0:  # 1800 grab
                        grab_date = current_date - dt.timedelta(days=1)
                    else:  # 0315, 0900, 1230 grabs
