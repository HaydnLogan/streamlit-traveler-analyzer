...
from custom_range_calculator_1125_20 import (
...
st.set_page_config(
    page_title="Swing Analysis Tool 05e_20",
    page_icon="",
    layout="wide"
)
...
def main():
    st.title(" Market Swing Analysis Tool 05e_20 Two Pass")
            ...
            try:
                ...
                if st.button(" Generate Cluster Tables", key="gen_cluster_tables", type="primary"):
                    with st.spinner("Generating cluster tables with two-pass processing..."):
                        ...
                        # NEW: Recips DP (reverse arrival order)
                        # Pass 1: PRecip (abs > 40)
                        RECIPS_DP_PASS1 = {41, -41, 42, -42, 43, -43, 45, -45, 50, -50, 54, -54, 55, -55, 60, -60, 67, -67, 68, -68, 77, -77, 87, -87, 96, -96, 103, -103, 107, -107, 111, -111}
                        # Pass 2: DRecip (abs < 40)
                        RECIPS_DP_PASS2 = {1, -1, 2, -2, 3, -3, 5, -5, 6, -6, 10, -10, 14, -14, 15, -15, 22, -22, 27, -27, 30, -30, 36, -36, 38, -38, 39, -39}
                        ...
                        if not fogz_prep.empty:
                            ...
                            fogz_matches = match_cluster_table_entries(
                                ...
                                table_name="Fogz PD",  # Updated name
                        ...
                        if not ld_prep.empty:
                            ...
                            ld_matches = match_cluster_table_entries(
                                ...
                                max_output_spread=max_spread,
                                allow_mixed_feed=(match_type_selection == "Allow mixed feed"),
                                table_name="Lrg Disc PD",  # Updated name
                        ...
                        if not recips_prep.empty:
                            ...
                            # NEW MODEL: Recips DP (reverse arrival order of Recips PD)
                            st.info("⏳ Matching Recips DP entries (Premium first, then Discount)...")
                            recips_dp_matches = match_cluster_table_entries(
                                prep_df=recips_prep,
                                valid_list_pass1=RECIPS_DP_PASS1,  # PRecip (abs > 40) in Pass 1
                                valid_list_pass2=RECIPS_DP_PASS2,  # DRecip (abs < 40) in Pass 2
                                max_output_spread=max_spread,
                                measurement_df=measurement_df,
                                check_recip=True,  # Same reciprocal checking as Recips PD
                                allow_mixed_feed=(match_type_selection == "Allow mixed feed"),
                                table_name="Recips DP",  # Shown as "Recips DP" in Category column
                                feed_opens=recips_summary.get('feed_opens', {})
                            )
                            cluster_results['recips_dp_combined'] = recips_dp_matches
                            st.success(f"[OK] Recips DP matches: {len(recips_dp_matches)} found (verified reciprocals)")
                        else:
                        ...
                        cluster_results['timings'] = {
                            'fogz': fogz_time,
                            'fogz_dd': 0,  # Included in fogz timing
                            'large_discounts': ld_time,
                            'recips_pd': recips_time,
                            'recips_dp': 0,  # Included in recips timing
                            'total': total_time
                        }
                ...
                if 'cluster_tables' in st.session_state:
                    ...
                    if (('fogz_combined' in cluster_results and len(cluster_results.get('fogz_combined', [])) > 0) or
                        ...
                        try:
                            ...
                            st.caption("Excel file contains 6 tabs: Fogz PD, Fogz DD, Lrg Disc PD, Recips PD, Recips DP, and Combined (all tables sorted by Arrival_Output)")
                        
                        except Exception as e:
                            st.error(f"Error creating Excel file: {e}")
                    
                    # Helper function for highlighting based on Open position
                    def highlight_open_position(row):
                        """Highlight entire rows based on position relative to Open markers.
                        - Open rows and rows between opens: Yellow
                        - Rows above open: Light red
                        - Rows below open: Light blue
                        """
                        # This is a placeholder - will be applied after identifying open positions
                        return [''] * len(row)
                    
                    # Helper function for highlighting Arrival_Brackets column only
                    def highlight_arrival_brackets(df):
                        """Highlight only the Arrival_Brackets column based on bracket values.
                        - Yellow for [0] (today)
                        ...
                        def apply_bracket_color(row):
                            ...
                            if 'Arrival_Brackets' in df.columns:
                                ...
                                if '[0]' in bracket_val:
                                    colors[bracket_idx] = 'background-color: #fff9c4'  # Yellow for today
                                elif any(x in bracket_val for x in ['[-1]', '[-2]', '[-3]']):
                                    colors[bracket_idx] = 'background-color: #bbdefb'  # Blue for recent
                            return colors
                    ...
                    with st.expander(" FOGZ Matched Results", expanded=True):
                        if 'fogz_combined' in cluster_results and len(cluster_results['fogz_combined']) > 0:
                            ...
                            **FOGZ M#s:** {0, +/-1, +/-2, +/-3, +/-5, +/-6} matched with **PwX2_1_0:** {+/-40, +/-41, +/-43, +/-50, +/-55, +/-60, +/-68, +/-77, +/-87, +/-96, +/-103, +/-107, +/-111}
                            ...
                            styled_fogz = cluster_results['fogz_combined'].style.apply(
                                highlight_arrival_brackets(cluster_results['fogz_combined']), 
                                axis=1
                            ).format({
                                'Arrival_Output': '{:.3f}',
                                'Prox': '{:.3f}'
                    ...
                    with st.expander(" Fogz DD Matched Results (FOGZ + Large Discounts)", expanded=False):
                        if 'fogz_dd_combined' in cluster_results and len(cluster_results.get('fogz_dd_combined', [])) > 0:
                            ...
                            **FOGZ M#s (Pass 1):** {0, +/-1, +/-2, +/-3, +/-5, +/-6} matched with **Large Discount M#s (Pass 2):** {+/-36, +/-38, +/-39}
                            """)
                            
                            styled_fogz_dd = cluster_results['fogz_dd_combined'].style.apply(
                                highlight_arrival_brackets(cluster_results['fogz_dd_combined']), 
                                axis=1
                            ).format({
                                'Arrival_Output': '{:.3f}',
                                'Prox': '{:.3f}'
                            })
                            st.dataframe(styled_fogz_dd, use_container_width=True, height=400)
                            st.caption(f" Total Fogz DD Matches: {len(cluster_results['fogz_dd_combined'])}")
                        else:
                            st.info("No Fogz DD matches found within current spread settings.")
                    ...
                    with st.expander(" Large Discounts Matched Results", expanded=False):
                        if 'ld_combined' in cluster_results and len(cluster_results['ld_combined']) > 0:
                            ...
                            **Large Discount M#s:** {+/-36, +/-38, +/-39} matched with **PX2_1_0:** {+/-41, +/-43, +/-50, +/-55, +/-60, +/-68, +/-77, +/-87, +/-96, +/-103, +/-107, +/-111}
                            ...
                            styled_ld = cluster_results['ld_combined'].style.apply(
                                highlight_arrival_brackets(cluster_results['ld_combined']), 
                                axis=1
                            ).format({
                                'Arrival_Output': '{:.3f}',
                                'Prox': '{:.3f}'
                    ...
                    with st.expander(" Recips PD Matched Results", expanded=False):
                        if 'recips_combined' in cluster_results and len(cluster_results['recips_combined']) > 0:
                            ...
                            **Matched with R# mates:**
                            ...
                            styled_recips = cluster_results['recips_combined'].style.apply(
                                highlight_arrival_brackets(cluster_results['recips_combined']), 
                                axis=1
                            ).format({
                                'Arrival_Output': '{:.3f}',
                                'Prox': '{:.3f}'
                    ...
                    with st.expander(" Recips DP Matched Results (Premium→Discount)", expanded=False):
                        if 'recips_dp_combined' in cluster_results and len(cluster_results.get('recips_dp_combined', [])) > 0:
                            ...
                            **PRecip List (abs > 40):** {+/-41, +/-42, +/-43, +/-45, +/-50, +/-54, +/-55, +/-60, +/-67, +/-68, +/-77, +/-87, +/-96, +/-103, +/-107, +/-111}
                            
                            **Matched with DRecip mates (abs < 40):**
                            - X0: (30,50), (22,60), (14,68), (10,77), (6,87), (5,96), (3,103), (2,107), (1,111)
                            - XD0: (27,54), (15,67)
                            - X1: (36,43), (26,55)
                            - XD1: (33,45)
                            ...
                            styled_recips_dp = cluster_results['recips_dp_combined'].style.apply(
                                highlight_arrival_brackets(cluster_results['recips_dp_combined']), 
                                axis=1
                            ).format({
                                'Arrival_Output': '{:.3f}',
                                'Prox': '{:.3f}'
                            })
                            st.dataframe(styled_recips_dp, use_container_width=True, height=400)
                            st.caption(f" Total Recip DP Matches: {len(cluster_results['recips_dp_combined'])}")
                        else:
                            st.info("No Recip DP matches found within current spread settings.")
