...
def match_cluster_table_entries(prep_df, valid_list_pass1, valid_list_pass2, max_output_spread=3.0, feed_filter=None, measurement_df=None, check_recip=False):
    ...
    if check_recip and measurement_df is not None:
        try:
            ...
            recip_col = next((c for c in measurement_df.columns if c.lower().replace(' ', '') in ['recipr#', 'recipr', 'reciprocal']), None)
            ...
            if m_col and recip_col:
                for _, row in measurement_df.iterrows():
                    ...
                    if pd.notna(m_num) and pd.notna(recip_num):
                        try:
                            recip_lookup[int(float(m_num))] = int(float(recip_num))
                        except:
                            pass
        except Exception as e:
            st.warning(f"Could not build reciprocal lookup: {e}")
        ...
        for _, row1 in pass1_df.iterrows():
            ...
            for _, row2 in pass2_df.iterrows():
                ...
                if check_recip and recip_lookup:
                    ...
                    if not (expected_recip_of_m1 == m2_int or expected_recip_of_m2 == m1_int):
