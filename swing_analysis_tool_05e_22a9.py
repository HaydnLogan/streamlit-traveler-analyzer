...
st.set_page_config(
    page_title="Swing Analysis Tool 05e_22a9",
    page_icon="",
    layout="wide"
)
...
def main():
    st.title(" Market Swing Analysis Tool 05e_22a9 Two Pass")
                    ...
                    if not bypass_mode_active:
                        ...
                        with st.spinner("Processing all 23 trading models using uploaded traveler report..."):
                            ...
                            try:
                                ...
                                # Now process both Model X and 23 Trading Models
                                st.markdown("---")
                                st.write("\n🔬 **Analysis Options**")
                                ...
                                with col1:
                                    # Model X Detection button
                                    if st.button("🧪 Run Model X Detection", key="run_model_x", type="primary"):
                                        try:
                                            from mod_x_03g import run_x_model_detection
                                            st.write("🔍 Analyzing traveler sequences...")
                                            run_x_model_detection(small_hlc_df)
                                        except ImportError as e:
                                            st.error(f"❌ Cannot import Model X detector: {str(e)}")
                                            st.info("Please ensure mod_x_03g.py is available in the application directory.")
                                        except Exception as e:
                                            st.error(f"❌ Error running Model X detection: {str(e)}")
                                            import traceback
                                            with st.expander("🔍 Error Details"):
                                                st.code(traceback.format_exc())
                                
                                with col2:
                                    # 23 Trading Models button
                                    if st.button("🎯 Run 23 Trading Models", key="run_23_models", type="primary"):
                                        try:
                                            st.write("🔍 Matching travelers for 23 trading models...")
                                            ...
                                            for model_name, model_def in MODELS.items():
                                                ...
                                                model_config = {
                                                    'pass1_ms': pass1_ms,
                                                    'pass2_ms': pass2_ms,
                                                    'day_filter': '[0]',  # Day 0 for Pass 1
                                                    'feed_selection': 'both'
                                                }
                                                
                                                # Match travelers using bypass mode matcher
                                                matched_df = process_model_bypass_mode(
                                                    small_hlc_df,
                                                    model_config,
                                                    max_spread=3.0
                                                )
                                                ...
                                                all_results['timings'][model_name] = time_module.time() - model_start
                                            
                                            all_results['total_time'] = time_module.time() - total_start
                                            
                                            # Display summary
                                            st.success(f"✅ Processed 23 models in {all_results['total_time']:.1f}s!")
                                                    ...
                                                    with st.expander(
                                                        ...
                                                        if len(matched_df) > 0:
                                                            ...
                                                            **Legend:** 
                                                            ...
                                                            timing = all_results['timings'].get(model_name, 0)
                                                            st.caption(f"📊 {len(matched_df)} matches | ⏱️ Generated in {timing:.2f}s")
                                                        else:
                                                            st.info("No matches found within current spread settings.")
                                        
                                        except ImportError as e:
                                            st.error(f"❌ Cannot import required modules: {str(e)}")
                                            st.info("Please ensure bypass_mode_matcher.py and model_definitions_v21.py are available.")
                                        except Exception as e:
                                            st.error(f"❌ Error running 23 models: {str(e)}")
                                            import traceback
                                            with st.expander("🔍 Error Details"):
                                                st.code(traceback.format_exc())
