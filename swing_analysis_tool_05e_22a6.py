...
def main():
    st.title(" Market Swing Analysis Tool 05e_22a6 Two Pass")
                ...
                if st.button("🚀 Process All Models", key="gen_all_models", type="primary"):
                    if not bypass_mode_active:
                        ...
                        with st.spinner("Processing all 23 trading models using uploaded traveler report..."):
                            ...
                            try:
                                # The uploaded traveler report from main app should already have:
                                # - All M# and R# values
                                ...
                                # Check if data is already processed (has paired structure)
                                # If it has columns like 'M #', 'R #', 'Origin', 'Arrival', etc., it's raw traveler data
                                # If it has columns like 'Input1', 'Input2', 'Prox', it's already processed
                                
                                is_already_processed = any(col in small_hlc_df.columns for col in ['Input1', 'Input2', 'Prox', 'Input_1', 'Input_2'])
                                
                                if is_already_processed:
                                    st.info("✓ Data appears to be already processed with pairs matched")
                                    cluster_df = small_hlc_df.copy()
                                else:
                                    # Raw traveler data - needs Pass 1 & Pass 2 processing
                                    st.write("🔄 Applying Pass 1 & Pass 2 cluster table processing...")
                                    
                                    # We need measurement_df for Pass 1 & Pass 2
                                    # Since we don't have it in bypass mode, create minimal one or skip
                                    st.warning("⚠️ Note: Bypass mode works best with pre-processed traveler reports from main app")
                                    st.info("💡 Attempting to use data as-is for model matching...")
                                    
                                    # For now, use the raw data directly
                                    # The 23 models will attempt to work with whatever structure is available
                                    cluster_df = small_hlc_df.copy()
                                
                                # Separate by feed if present (for statistics)
                                if 'Feed' in cluster_df.columns:
                                    small_count = len(cluster_df[cluster_df['Feed'] == 'Small'])
                                    big_count = len(cluster_df[cluster_df['Feed'] == 'Big'])
                                    st.write(f"  📊 Small feed: {small_count} rows")
                                    st.write(f"  📊 Big feed: {big_count} rows")
                                    st.write(f"  📊 Total: {len(cluster_df)} rows")
                                else:
                                    st.write(f"  📊 Total data: {len(cluster_df)} rows")
                                ...
                                if 'Arrival_Output' in cluster_df.columns:
                                    ...
                                    st.success(f"📊 Output Coverage: {output_min:.2f} to {output_max:.2f} (Spread: {output_spread:.2f} units)")
                                
                                # Now run all 23 models on the data
                                st.write("\n🎯 Running 23 Trading Models...")
                                
                                # For bypass mode, we pass the data directly
