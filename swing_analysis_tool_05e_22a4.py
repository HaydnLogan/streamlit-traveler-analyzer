...
st.set_page_config(
    page_title="Swing Analysis Tool 05e_22a",
    page_icon="",
    layout="wide"
)
...
def main():
    st.title(" Market Swing Analysis Tool 05e_22a4 Two Pass")
            ...
            try:
                ...
                if st.button("🚀 Process All Models", key="gen_all_models", type="primary"):
                    if not bypass_mode_active:
                        ...
                        # BYPASS MODE - Use uploaded traveler report with full cluster table processing
                        with st.spinner("Processing all 23 trading models using uploaded traveler report..."):
                            st.info("⚡ Bypass Mode: Using pre-generated traveler data with Pass 1 & Pass 2 processing")
                            
                            try:
                                # The uploaded traveler report is a raw cluster table that needs
                                # Pass 1 and Pass 2 processing to match pairs according to model rules
                                ...
                                # Apply two-pass cluster table processing
                                st.write("🔄 Applying Pass 1 & Pass 2 cluster table processing...")
                                
                                # Import the two-pass processing function
                                from custom_range_calculator_1125_21c import process_cluster_tables_two_pass
                                ...
                                if 'Feed' in small_hlc_df.columns:
                                    ...
                                    # Process each feed through two-pass logic
                                    # Process small feed
                                    ...
                                    # Combine both feeds
                                    cluster_df = pd.concat([small_processed, big_processed], ignore_index=True)
                                    
                                    st.success(f"✅ After Pass 1 & 2: {len(cluster_df)} total matched pairs")
                                else:
                                ...
                                if 'Arrival_Output' in cluster_df.columns:
                                    output_min = cluster_df['Arrival_Output'].min()
                                    output_max = cluster_df['Arrival_Output'].max()
                                    output_spread = output_max - output_min
                                    
                                    st.info(f"📊 Output Coverage: {output_min:.2f} to {output_max:.2f} (Spread: {output_spread:.2f} units)")
                                
                                # Now run all 23 models on the processed cluster table
                                st.write("\n🎯 Running 23 Trading Models on matched pairs...")
                                
                                # For bypass mode, we pass the processed cluster table directly
                                bypass_measurement_df = pd.DataFrame()
                                
                                # Import model processor
                                from model_processor_v21 import process_all_models
                                
                                # Process models - the cluster_df is our processed cluster table
                                # We pass it as small_df since it's already processed
                                all_results = process_all_models(
                                    measurement_df=bypass_measurement_df,
                                    small_df=cluster_df,  # This is the processed cluster table
                                    big_df=None,  # Already combined if needed
                                    report_time=report_time,
                                    lookback_days=None,  # Not needed - data already filtered
                                    max_spread=None,  # Not needed - data already matched
                                    window_radius=None,  # Not needed - data already windowed
                                    allowed_origins=None,  # All origins in report
                                    segment_size=None,  # Not segmenting
                                    combine_segments=True,
                                    feed_selection="Combined",
                                    match_type_selection="Same feed only"
                                )
                                
                            except ImportError as e:
                                st.error(f"❌ Missing required module: {str(e)}")
                                st.error("Bypass mode requires the custom_range_calculator_1125_21c module for Pass 1 & Pass 2 processing.")
