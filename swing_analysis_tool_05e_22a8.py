...
# Import Excel exporter with highlighting
from excel_exporter_v21 import create_download_button
...
st.set_page_config(
    page_title="Swing Analysis Tool 05e_22a8",
    page_icon="",
    layout="wide"
)
...
def main():
    st.title(" Market Swing Analysis Tool 05e_22a8 Two Pass")
                ...
                if st.button("🚀 Process All Models", key="gen_all_models", type="primary"):
                    if not bypass_mode_active:
                        ...
                        with st.spinner("Processing all 23 trading models..."):
                            ...
                            # Excel Export with Report Time and highlighting
                            st.markdown("---")
                            st.markdown("### 📥 Export Results")
                            create_download_button(all_results, report_time)
                        ...
                        with st.spinner("Processing all 23 trading models using uploaded traveler report..."):
                            ...
                            try:
                                # Rename 'Output' column to 'Arrival_Output' if needed for compatibility
                                if 'Output' in small_hlc_df.columns and 'Arrival_Output' not in small_hlc_df.columns:
                                    small_hlc_df = small_hlc_df.rename(columns={'Output': 'Arrival_Output'})
                                ...
                                if 'Feed' in small_hlc_df.columns:
                                    ...
                                    # Show output range for each feed
                                    if 'Arrival_Output' in small_feed_df.columns:
                                        small_min = small_feed_df['Arrival_Output'].min()
                                        small_max = small_feed_df['Arrival_Output'].max()
                                        st.write(f"🧮 Small Feed Range: [{small_min:.2f}, {small_max:.2f}]")
                                    
                                    if 'Arrival_Output' in big_feed_df.columns:
                                        big_min = big_feed_df['Arrival_Output'].min()
                                        big_max = big_feed_df['Arrival_Output'].max()
                                ...
                                if 'Arrival_Output' in small_hlc_df.columns:
                                    output_min = small_hlc_df['Arrival_Output'].min()
                                    output_max = small_hlc_df['Arrival_Output'].max()
                                ...
                                # Now run all 23 models
                                st.write("\n🎯 Running 23 Trading Models...")
                                
                                # Import model processor
                                from model_processor_v21 import process_all_models
                                
                                # For bypass mode, we need to provide window_radius and other parameters
                                # Calculate window_radius from the output range in the traveler report
                                if 'Arrival_Output' in small_hlc_df.columns:
                                    output_min = small_hlc_df['Arrival_Output'].min()
                                    output_max = small_hlc_df['Arrival_Output'].max()
                                    output_range = output_max - output_min
                                    
                                    # Window radius is half the range
                                    calculated_window_radius = int(output_range / 2)
                                    
                                    # Feed start is the midpoint
                                    calculated_feed_start = (output_min + output_max) / 2
                                    
                                    st.info(f"💡 Calculated window parameters from report: radius={calculated_window_radius}, center={calculated_feed_start:.2f}")
                                else:
                                    # Fallback to reasonable defaults
                                    calculated_window_radius = 600
                                    calculated_feed_start = None
                                    st.warning("⚠️ Could not calculate window parameters, using defaults")
                                
                                # In bypass mode, the traveler report IS the raw data
                                # We need to let process_all_models know to generate cluster tables from it
                                # But provide the calculated parameters
                                
                                try:
                                    # Process models with calculated parameters
                                    all_results = process_all_models(
                                        measurement_df=bypass_measurement_df,  # M#s from report
                                        small_df=cluster_small,                 # Small feed data
                                        big_df=cluster_big,                     # Big feed data or None
                                        report_time=report_time,
                                        lookback_days=20,  # Reasonable default
                                        max_spread=3.0,    # Reasonable default
                                        window_radius=calculated_window_radius,  # Calculated from report
                                        allowed_origins=None,  # Process all origins in report
                                        segment_size=None,     # No segmentation
                                        combine_segments=True,
                                        feed_selection="Both feeds" if cluster_big is not None else "Small feed only",
                                        match_type_selection="Same feed only"
                                    )
                                except Exception as e:
                                    st.error(f"❌ Error running models: {str(e)}")
                                    st.warning("⚠️ The traveler report structure may not be compatible with model processing.")
                                    st.info("💡 Bypass mode expects a specific traveler report format with columns: Feed, M #, Origin, Arrival_Output, etc.")
                                    
                                    with st.expander("🔍 DEBUG: Model Processing Error"):
                                        import traceback
                                    ...
                                    summary_stats = {
                                        'total_models': 23,
                                        'models_with_matches': 0,
                                        'total_matches': 0
                                    }
                                    # Don't return, let it continue to show the error state
                                else:
                                    # Success - create summary stats
                                    summary_stats = create_summary_stats(all_results)
                                
                            except ImportError as e:
                                st.error(f"❌ Missing required module: {str(e)}")
                                st.info("Please ensure all required modules are available in the application directory.")
                                return
                            except Exception as e:
                            ...
                            # Display summary (summary_stats already created in try block)
                            col1, col2, col3, col4 = st.columns(4)
                            with col1:
                                st.metric("Total Models", summary_stats['total_models'])
                            with col2:
                            ...
                            with col3:
                                st.metric("Total Matches", summary_stats['total_matches'])
                            with col4:
                                processing_time = all_results.get('total_time', 0)
                                st.metric("Processing Time", f"{processing_time:.1f}s")
                            
                            if summary_stats['total_matches'] > 0:
                                st.success(f"✅ Processed {summary_stats['total_models']} models in {processing_time:.1f}s!")
                            else:
                                st.warning(f"⚠️ Processed {summary_stats['total_models']} models but found no matches")
                            ...
                            if summary_stats['total_matches'] > 0:
                                ...
                                st.warning("⚠️ No matches found across all models. Nothing to export.")
                                st.info("💡 This could mean:")
                                st.markdown("""
                                - The traveler report data structure doesn't match what models expect
                                - The M# values don't form valid recipe pairs
                                - The output values are outside expected ranges
                                - Try checking the DEBUG expander below for data structure details
                                """)
                                
                                # Show data structure for debugging
                                with st.expander("🔍 DEBUG: Traveler Report Structure"):
                                    st.write("**Columns in traveler report:**")
                                    st.code(str(list(small_hlc_df.columns)))
                                    st.write("**Sample M# values:**")
                                    if 'M #' in small_hlc_df.columns:
                                        st.code(str(sorted(small_hlc_df['M #'].unique())[:20]))
                                    st.write("**First 5 rows:**")
                                    st.dataframe(small_hlc_df.head())
                                    st.write("**Data types:**")
                                    st.code(small_hlc_df.dtypes.to_string())
