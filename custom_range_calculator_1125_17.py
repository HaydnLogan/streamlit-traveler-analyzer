...
v1125_15. THREE ENHANCEMENTS: Layout, formatting, negative reciprocals.

1. NEGATIVE RECIPROCAL FIX:
   - Issue: Valid pairs like (6, -87) and (39, -41) appeared in FOGZ/LD but not Recips PD
   - Cause: Reciprocal checking only matched exact values, not considering sign
   - Fix: Now checks absolute values too
     * If 6 and 87 are reciprocals, then:
       - 6 ↔ 87 ✓
       - 6 ↔ -87 ✓ (NEW)
       - -6 ↔ 87 ✓ (NEW)
       - -6 ↔ -87 ✓ (NEW)
   - Impact: All valid reciprocal pairs now appear in Recips PD

2. DISPLAY FORMATTING:
   - Truncated Arrival_Output and Prox columns to 3 decimal places
   - Display only (underlying values unchanged)
   - Format: 24365.167 instead of 24365.16666666667
   - Applied to FOGZ, Large Discounts, and Recips PD tables

3. LAYOUT IMPROVEMENT (in swing tool):
   - Restructured Processing Mode section into 3 columns:
     * Column 1: Window Radius slider
     * Column 2: Feed Selection radio buttons
     * Column 3: Match Type radio buttons
   - Status line appears below all three columns
   - More compact layout, better use of screen space

v1125_14. TWO FEATURES ADDED: Feed filtering and reciprocal column fix.

1. FEED SELECTION FEATURE:
   - Added feed_selection parameter: "Both feeds", "Small feed only", "Big feed only"
   - UI control in Processing Mode section
   - Only processes selected feed(s)
   - Reduces processing time when only one feed needed
   - Example: "Small feed only" skips Big feed entirely

2. MIXED FEED MATCHING CONTROL:
   - Added allow_mixed_feed parameter (default=True for backward compatibility)
   - UI control: "Same feed only" vs "Allow mixed feed"
   - "Same feed only": Matches within each feed separately (Small with Small, Big with Big)
   - "Allow mixed feed": Allows cross-feed matches (Small with Big)
   - Applied to all three tables: FOGZ, Large Discounts, Recips PD

3. RECIPROCAL COLUMN FIX:
   - Fixed: "⚠️ Could not find reciprocal columns. M# column: M #, Recip column: None"
   - Added 'r#' and 'r' to column matching (was only looking for 'recipr#', 'recipr', etc.)
   - Now detects "R #" column correctly
   - Should fix invalid reciprocal pairs appearing in results

v1125_13. THREE CRITICAL FIXES: Arrival output, feed mismatch, recip debugging.

1. ARRIVAL_OUTPUT FIX:
   - Changed from averaging two outputs to using Pass1 (arrival) output
   - Old: zone_price = (output1 + output2) / 2
   - New: arrival_output = output1 (the actual arrival output)
   
2. FEED DISPLAY FIX:
   - Was only showing Feed1 in matches
   - Now shows "Small + Big" when feeds are different
   - Added Feed1 and Feed2 columns for debugging
   - Fixes issue where Kepler-62 M#-40 from Big feed showed as Small feed match
   
3. RECIPROCAL CHECKING DEBUG:
   - Added extensive debug output for reciprocal lookup building
   - Shows column names found, number of entries, example pairs
   - Shows matched pairs after matching
   - Helps diagnose why wrong pairs still appearing
   - Improved column name matching to include 'reciprocal#'

v1125_12. THREE CRITICAL FIXES: Big feed results, Recip pair validation, highlighting.
1. BIG FEED FIX: Changed deduplication from ['M #', 'Origin', 'Arrival'] to include 'Feed'
   - Big feed results were being dropped as duplicates
   - Now both Small and Big feed results preserved
   
2. RECIPS PD FIX: Added reciprocal relationship validation
   - Added check_recip parameter to match_cluster_table_entries()
   - Reads 'Recip R #' column from measurement file
   - Verifies M# pairs are actual reciprocals (e.g., M#5 ↔ M#96)
   - Prevents invalid pairs like (5, 42) or (5, 67)
   
3. HIGHLIGHTING FIX: Changed from 'Type' column to 'Arrival_Bracket' column
   - Yellow: [0] (today arrivals)
   - Blue: [-1], [-2], [-3] (recent arrivals)
   - No highlight: Older arrivals
   - Was highlighting everything yellow including [-3] arrivals

v1125_11. CRITICAL BUG FIX: Origin loop indentation corrected.
- Fixed catastrophic bug where origin processing was OUTSIDE the origin loop
- This caused only the LAST origin in the list to be processed
- Symptoms: Only one origin showing results at a time, no cross-origin matches
- When unchecking one origin, next one would appear (processing in reverse order)
- Results only from small feed (big feed ignored after first origin)
- Fix: Indented lines 807-946 to be inside "for origin_name in feed_origins:" loop
- All origins now process correctly, cross-feed and cross-origin matches work

v1125_10. CRITICAL FIX: Dynamic origin detection and matching.
- Added debug output showing all origins found in each feed before filtering
- Enhanced origin filtering display to show matches vs mismatches
- Shows "Found X origins" and "Processing Y/X filtered origins"
- Warnings when no origins match filters (with details on available vs allowed)
- This fixes the issue where only Macedonia was being recognized
...
- Each segment processes independently, reducing memory and processing time
- combine_segments parameter controls whether to merge results or keep separate
- Particularly effective for large datasets with many origins

Example: Window radius 150 → 300 units total
- Without segmentation: Process all 300 units at once
- With 75-unit segments: Process 4×75 unit chunks (reduces memory by ~75%)
- Results can be combined automatically or analyzed separately

v1125_8. PERFORMANCE: Adds origin filtering to handle large datasets.
- Increased Pandas styler max cells from 262K to 1M
- Added origin filtering UI with Anchor/Epic as non-negotiable priority
- Optional additional origins: Spain, Saturn, Jupiter, WASP-12b, Macedonia, Fiji, Charlie
- Filtering applied early in processing for speed improvement
- Shows filtered origins count per feed

With filtering, expected time improvements:
...
def match_cluster_table_entries(prep_df, valid_list_pass1, valid_list_pass2, max_output_spread=3.0, feed_filter=None, measurement_df=None, check_recip=False, allow_mixed_feed=True, table_name="Cluster", feed_opens=None):
    ...
    try:
        ...
        # ACTION ITEM 2: Detect multiple pairs (confluence)
        # Group by same Arrival_Output, Arrival_DateTime, and Feed1 to find multiple pairs
        matches_df['Confluence'] = ''
        
        if not matches_df.empty:
            # Group by the key that identifies multiple pairs
            grouped = matches_df.groupby(['Arrival_Output', 'Arrival_DateTime', 'Feed1'])
            
            for (output, dt, feed), group_df in grouped:
                if len(group_df) > 1:
                    # Multiple pairs found
                    num_pairs = len(group_df)
                    
                    # Determine group classification
                    groups_in_set = group_df['Group'].unique()
                    
                    # Check if all same group
                    if len(groups_in_set) == 1:
                        group_code = groups_in_set[0].split('.')[1].strip()  # Extract SAA, AA, etc.
                        confluence_label = f"{num_pairs} pr {group_code}"
                    else:
                        # Mixed groups - check patterns
                        group_codes = [g.split('.')[1].strip() for g in groups_in_set]
                        
                        # Check if all are SAA
                        if all(g == 'SAA' for g in group_codes):
                            confluence_label = f"{num_pairs} pr SAA"
                        # Check if all are AA
                        elif all(g == 'AA' for g in group_codes):
                            confluence_label = f"{num_pairs} pr AA"
                        # Check if mix of Anchor-related (SAA, AA, TA, AT, oA, Ao)
                        elif all(g in ['SAA', 'AA', 'TA', 'AT', 'oA', 'Ao'] for g in group_codes):
                            confluence_label = f"{num_pairs} pr AA"  # Mixed anchors
                        else:
                            confluence_label = f"{num_pairs} pr"
                    
                    # Apply to all rows in this group
                    matches_df.loc[group_df.index, 'Confluence'] = confluence_label
