...
st.set_page_config(
    page_title="Swing Analysis Tool 05e_3",
    page_icon="📊",
    layout="wide"
)
            ...
            try:
                ...
                if st.button("🎯 Generate Cluster Tables", key="gen_cluster_tables", type="primary"):
                    with st.spinner("Generating cluster tables with two-pass processing..."):
                        ...
                        fogz_prep = process_cluster_tables_two_pass(
                        ...
                        ld_prep = process_cluster_tables_two_pass(
                            measurement_df=measurement_df,
                            small_df=small_hlc_df,
                            big_df=big_hlc_df,
                            report_time=report_time,
                        ...
                        if not ld_prep.empty:
                            ...
                            prep_tables['ld_prep'] = pd.DataFrame()
                        ...
                        recips_prep = process_cluster_tables_two_pass(
                            measurement_df=measurement_df,
                            small_df=small_hlc_df,
                            big_df=big_hlc_df,
                            report_time=report_time,
                        ...
                        if not recips_prep.empty:
                            ...
                            prep_tables['recips_prep'] = pd.DataFrame()
                ...
                if 'prep_tables' in st.session_state:
                    ...
                    with st.expander("🔍 FOGZ Prep Table", expanded=False):
                        if 'fogz_prep' in prep_tables and not prep_tables['fogz_prep'].empty:
                            st.markdown(f"**Pass 1 M#s (Recent 2 days):** {list(sorted(set(prep_tables['fogz_prep'][prep_tables['fogz_prep']['M #'].isin({0, 1, -1, 2, -2, 3, -3, 5, -5, 6, -6})]['M #'].unique())))}")
                            st.markdown(f"**Pass 2 M#s (Within lookback):** {list(sorted(set(prep_tables['fogz_prep'][~prep_tables['fogz_prep']['M #'].isin({0, 1, -1, 2, -2, 3, -3, 5, -5, 6, -6})]['M #'].unique())))}")
                    ...
                    with st.expander("🔍 Large Discounts Prep Table", expanded=False):
                        if 'ld_prep' in prep_tables and not prep_tables['ld_prep'].empty:
                            st.markdown(f"**Pass 1 M#s (Recent 2 days):** {list(sorted(set(prep_tables['ld_prep'][prep_tables['ld_prep']['M #'].isin({36, -36, 38, -38, 39, -39})]['M #'].unique())))}")
                            st.markdown(f"**Pass 2 M#s (Within lookback):** {list(sorted(set(prep_tables['ld_prep'][~prep_tables['ld_prep']['M #'].isin({36, -36, 38, -38, 39, -39})]['M #'].unique())))}")
                    ...
                    with st.expander("🔍 Recips PD Prep Table", expanded=False):
                        if 'recips_prep' in prep_tables and not prep_tables['recips_prep'].empty:
                            pass1_set = {1, -1, 2, -2, 3, -3, 5, -5, 6, -6, 10, -10, 14, -14, 15, -15, 22, -22, 27, -27, 30, -30, 36, -36, 38, -38, 39, -39}
                            st.markdown(f"**Pass 1 M#s (Recent 2 days):** {list(sorted(set(prep_tables['recips_prep'][prep_tables['recips_prep']['M #'].isin(pass1_set)]['M #'].unique())))}")
                            st.markdown(f"**Pass 2 M#s (Within lookback):** {list(sorted(set(prep_tables['recips_prep'][~prep_tables['recips_prep']['M #'].isin(pass1_set)]['M #'].unique())))}")
