...
# ============================================================================
# DIAGNOSTIC VERSION - v21d
# Last Updated: December 5, 2025 - 1:30 PM EST
# ============================================================================
print("\n" + "=" * 80)
print("🔍 DIAGNOSTIC: custom_range_calculator_1125_21.py MODULE LOADED")
print("Version: v21d-diagnostic")
print("Timestamp: December 5, 2025 - 1:30 PM EST")
print("Cache Check: If you don't see this message, Python is using CACHED code!")
print("=" * 80 + "\n")
# ============================================================================
...
def match_cluster_table_entries(prep_df, valid_list_pass1, valid_list_pass2, max_output_spread=3.0, feed_filter=None, measurement_df=None, check_recip=False, allow_mixed_feed=True, table_name="Cluster", feed_opens=None):
    ...
    try:
        ...
        if not matches_df.empty:
            ...
            # DIAGNOSTIC: Check grouping for specific output
            diagnostic_output = 24561.92
            has_diagnostic_output = any(abs(matches_df['Arrival_Output'] - diagnostic_output) < 0.01)
            
            if has_diagnostic_output and table_name == "Lrg Disc PD":
                print("\n" + "🔍" * 40)
                print(f"DIAGNOSTIC: Processing confluence for {table_name}")
                print(f"Looking for output ~{diagnostic_output}")
                print(f"Total groups created: {len(grouped)}")
                
                # Show groups for our target output
                for (arr_out, feed, base_orig, base_m, dt), gdf in grouped:
                    if abs(arr_out - diagnostic_output) < 0.01:
                        unique_m2s = gdf['_match_m'].unique()
                        print(f"\n  📊 Group: origin={base_orig}, M#={base_m}")
                        print(f"     Rows: {len(gdf)}")
                        print(f"     Unique M2s: {sorted(unique_m2s)}")
                        print(f"     Count: {len(unique_m2s)} Pr")
                ...
                if len(group_df) >= 1:  # Process all groups, even single pairs
                    ...
                    for idx in group_df.index:
                        ...
                        if num_unique_matches > 0:
                            ...
                            # DIAGNOSTIC: Show what we're setting
                            if abs(arrival_output - 24561.92) < 0.01 and table_name == "Lrg Disc PD":
                                print(f"      ✅ Setting row {idx}: '{num_unique_matches} Pr' (origin={base_origin})")
