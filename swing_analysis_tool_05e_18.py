...
v05e_17. TWO ACTION ITEMS: Open/Zone/Confluence columns and multiple pairs detection.
...
v05e_16. SEVEN ACTION ITEMS: Tags/Families, column renames, Category, Pattern_Type, Group, timing, Excel export.

1. TAGS AND FAMILIES COLUMNS:
   - Added 'Tags' column to matched tables
   - Added 'Families' column to matched tables
   - Shows Pass1 values first, then Pass2 values
   - Format: "value1, value2"

2. COLUMN RENAMES:
   - OLD 'Category' → NEW 'Match' (M# pairs)
   - OLD 'Model' → NEW 'Category' (table names)

3. CATEGORY COLUMN (formerly Model):
   - Shows table-specific abbreviated names
   - Format: "Fogz PD", "Lrg Disc PD", "Recips PD"
   - PD = Premium before Discount
   - DP = Discount before Premium
   - Based on M# absolute values (< |40| = Discount, >= |40| = Premium)

4. PATTERN_TYPE COLUMN:
   - Updated with G.11 patterns
   - X0, XD0, X1, XD1, X2, XD2
   - "undef" for undefined patterns
   - All implemented in calculator

5. GROUP COLUMN:
   - Updated with G.11 classification
   - Groups 1-7 based on origin types
   - Anchor origins: Spain, Saturn, Jupiter, Kepler-62, Kepler-44
   - TT origins: Trinidad, Tobago
   - All implemented in calculator

6. TIMING DISPLAY UPDATED:
   - Changed from green "st.success" to orange "st.warning"
   - Replaced "[OK]" with "⏱️"
   - New text: "⏱️ **Timing Summary:** All cluster tables generated in X.XXs!"
   - Orange background for better visibility

7. EXCEL EXPORT BUTTON:
   - Added download button for Excel file
   - Button: "📥 Download All Matched Tables (Excel)"
   - 4 tabs in Excel file:
     * Tab 1: FOGZ (individual table)
     * Tab 2: Large Discounts (individual table)
     * Tab 3: Recips PD (individual table)
     * Tab 4: Combined (all tables merged, sorted by Arrival_Output)
   - Filename includes timestamp: cluster_tables_YYYYMMDD_HHMMSS.xlsx
   - Caption explains tab contents
   - Error handling for Excel creation

v05e_15. THREE ENHANCEMENTS: Layout, formatting, negative reciprocals.

1. LAYOUT IMPROVEMENT:
   - Restructured Processing Mode section into 3-column layout
   - Column 1: Window Settings (Window Radius slider)
   - Column 2: Feed Selection (which feed(s) to process)
   - Column 3: Match Type (cross-feed matching control)
   - Status line appears below all three columns
   - More compact, better use of horizontal space
   - Example: "✓ Processing both feeds, matching within each feed separately"

2. DISPLAY FORMATTING:
   - Added 3 decimal place formatting to matched tables
   - Columns: Arrival_Output and Prox
   - Format string: '{:.3f}'
   - Display: 24365.167 instead of 24365.16666666667
   - Underlying values unchanged (formatting is display-only)
   - Applied to all three tables: FOGZ, Large Discounts, Recips PD

3. NEGATIVE RECIPROCAL FIX (in calculator):
   - Issue: Valid pairs like (6, -87) and (39, -41) in FOGZ/LD but missing from Recips PD
   - Cause: Reciprocal checking only matched exact values
   - Fix: Now checks absolute values in addition to exact values
   - Logic: If M#6 has R#87, then all combinations are valid:
     * (6, 87) ✓
     * (6, -87) ✓
     * (-6, 87) ✓
     * (-6, -87) ✓
   - Impact: All valid reciprocal pairs now appear in Recips PD table

v05e_14. TWO FEATURES ADDED: Feed filtering controls and reciprocal column fix.

1. FEED SELECTION & MATCHING CONTROLS:
   - New UI section: "Feed Selection & Matching"
   - Feed Selection options:
     * "Both feeds" (default) - Process both Small and Big feeds
     * "Small feed only" - Only process Small feed
     * "Big feed only" - Only process Big feed
   - Match Type options:
     * "Same feed only" (default) - Match within each feed separately
     * "Allow mixed feed" - Allow matches across feeds (Small + Big)
   - Benefits:
     * Faster processing when only one feed needed
     * Control over cross-feed matches
     * Default prevents mixed feed results (as requested)

2. RECIPROCAL COLUMN DETECTION FIX:
   - Issue: Warning "⚠️ Could not find reciprocal columns. M# column: M #, Recip column: None"
   - Cause: Column matching didn't include 'R #' pattern
   - Fix: Added 'r#' and 'r' to column detection (was only checking 'recipr#', 'recipr', 'reciprocal')
   - Now correctly finds "R #" column in measurement file
   - Should resolve invalid reciprocal pairs appearing in results

3. FEED PROCESSING LOGIC:
   - feed_selection passed to process_cluster_tables_two_pass()
   - Only selected feed(s) processed
   - Shows info message: "📋 Processing X feed(s): Small, Big"
   - Empty results if no feeds selected

4. MATCH FILTERING LOGIC:
   - allow_mixed_feed passed to match_cluster_table_entries()
   - When False: Skips matches where feed1 != feed2
   - Applied to all three tables: FOGZ, LD, Recips PD
   - Maintains Feed1/Feed2 debug columns for verification

v05e_13. THREE CRITICAL FIXES: Arrival output, feed mismatch, recip debugging.

1. ARRIVAL_OUTPUT COLUMN FIX:
   - Issue: Column showed average of two matched outputs
   - User request: Show actual arrival output (not average)
   - Fix: Changed from (output1 + output2)/2 to just output1 (Pass1 entry)
   - Impact: Arrival_Output now shows the actual arrival value

2. FEED MISMATCH FIX:
   - Issue: "Kepler-62 M#-40 at 24,365.17 on Big feed used in match on Small feed"
   - Cause: Only showing feed1 in Feed column
   - Fix: Show "Small + Big" when feeds differ, added Feed1/Feed2 debug columns
   - Impact: Feed column accurately represents which feed(s) contributed

3. RECIPROCAL CHECKING DEBUG:
   - Issue: Wrong pairs still appearing despite reciprocal checking
   - Fix: Added extensive debug output to diagnose issue
     * Shows reciprocal column names found
     * Shows number of reciprocal entries loaded
     * Shows example pairs (e.g., [(5, 96), (1, 55), ...])
     * Shows matched pairs after filtering
   - Impact: Can now see exactly what reciprocal data is being used

v05e_12. THREE CRITICAL FIXES: Big feed results, Recip pairs, highlighting.

1. BIG FEED RESULTS FIX:
   - Only Small feed results showing in FOGZ and Large Discounts (Big feed in summary but not matches)
   - Cause: Deduplication removing Big feed results
   - Fix: Added 'Feed' column to drop_duplicates subset
   - Result: Both Small and Big feed matches now appear

2. RECIPS PD PAIR VALIDATION FIX:
   - Wrong pairs appearing: (5, 42), (5, 67) when M#5 Recip is M#96
   - Cause: match_cluster_table_entries() matching ANY Pass1 with ANY Pass2 with close outputs
   - Fix: Added reciprocal relationship checking using 'Recip R #' column from measurement file
   - Result: Only valid reciprocal pairs now matched (e.g., 5↔96, not 5↔42)

3. HIGHLIGHTING FIX:
   - Everything highlighted yellow, even [-3] arrivals
   - Cause: Using 'Type' column with date-based logic
   - Fix: Changed to 'Arrival_Bracket' column checking
     * Yellow: [0] (today)
     * Blue: [-1], [-2], [-3] (recent)
     * None: Older arrivals
   - Result: Correct color coding based on arrival brackets

v05e_11. CRITICAL BUG FIX: Origin loop indentation corrected.
- Fixed severe bug in custom_range_calculator where origin processing was outside the loop
- Symptoms fixed:
  * Only one origin processed at a time (the last one in the list)
  * When unchecking that origin, the next one appeared (reverse priority order)
  * No cross-origin matches ever found
  * Only small feed results showing (big feed ignored)
  * Macedonia → WASP-12b → Tobago → Trinidad... sequential appearance
- Cause: Lines 807-946 were not indented to be inside "for origin_name in feed_origins:"
- Impact: ALL origins now process, both feeds work, cross-origin matching restored

v05e_10. CRITICAL FIX: Dynamic origin detection from actual CSV columns.
- Origin filtering now dynamically reads origins from uploaded CSV files
- Automatically categorizes origins as Epic, Anchor, or Other (case-insensitive)
- Removes hardcoded origin lists (no more "Charlie" that doesn't exist)
- Shows "Detected X origins: Y Epic, Z Anchor, W Other" before filtering
- Uses exact origin names from CSV columns for matching (preserves case)
- Fixed issue where only Macedonia was recognized - now all origins work
- "Other Origins" list is now dynamic based on what's actually in your CSVs

v05e_9. WINDOW SEGMENTATION: Break large processing jobs into smaller segments.
- Added "Segment window into smaller ranges" checkbox
- Segment size slider (25-150 units, default 75)
- Automatically calculates number of segments based on window radius
- Option to combine segments or keep separate reports
- Fixes origin classifications: Epic (Trinidad, Tobago, WASP-12b, Macedonia) and Anchor (Spain, Saturn, Jupiter, Kepler-62, Kepler-44)

Benefits:
- Reduces memory usage by ~75% per segment
- Faster processing for each segment
- Better progress tracking (segment-by-segment)
- Can analyze segments individually if needed

Example: Window radius 150 with 75-unit segments:
- Full window: 300 units → 4 segments of 75 units each
- Segment 1: Units 0-75, Segment 2: 75-150, Segment 3: 150-225, Segment 4: 225-300
- Expected time: 12 min → 3-4 min per segment → Total ~12-16 min (but with progress indicators)

v05e_8. PERFORMANCE: Origin filtering to handle large datasets (563K+ cells).
- Increased Pandas styler.render.max_elements from 262K to 1M
- Added "Filter Origins" checkbox with Anchor and Epic as priority (always included)
- Optional additional origins: Spain, Saturn, Jupiter, WASP-12b, Macedonia, Fiji, Charlie
- Filtering reduces processing time by 40-60% and keeps results manageable
- Shows count of filtered origins being processed

Performance improvements (example with 5 origins vs all):
- FOGZ: 164s -> ~65-80s
- Large Discounts: 69s -> ~30-40s  
- Recips PD: 476s -> ~190-240s
- Result size: 563K cells -> ~100-200K cells
...
from custom_range_calculator_1125_18 import (
...
st.set_page_config(
    page_title="Swing Analysis Tool 05e_18",
    page_icon="",
    layout="wide"
)
...
def main():
    st.title(" Market Swing Analysis Tool 05e_18 Two Pass")
    ...
    with tab8:
        ...
        **High-Probability Turning Zones** identified through:
        ...
        if not min_files_met:
            st.warning("WARNING: Strategic Zones requires minimum 7 files:")
            st.markdown("""
            **Minimum Requirements:**
            ...
            try:
                ...
                if st.button(" Generate Cluster Tables", key="gen_cluster_tables", type="primary"):
                    with st.spinner("Generating cluster tables with two-pass processing..."):
                        ...
                        fogz_prep, fogz_summary = process_cluster_tables_two_pass(
                            ...
                            segment_size=segment_size if segment_window else None,
                            combine_segments=combine_segments if segment_window else True,
                        ...
                        with st.expander(" FOGZ Processing Summary", expanded=True):
                            ...
                            if fogz_summary.get('pass1_hlcs_examined', 0) > 0 or fogz_summary.get('pass2_hlcs_examined', 0) > 0:
                                st.markdown("---")
                                st.markdown("**DEBUG: Detailed HLC Examination:**")
                        ...
                        if not fogz_prep.empty:
                            st.success(f"[OK] FOGZ prep: {len(fogz_prep)} entries")
                        ...
                        if not ld_prep.empty:
                            st.success(f"[OK] Large Discounts prep: {len(ld_prep)} entries")
                            ...
                            ld_matches = match_cluster_table_entries(
                                ...
                                allow_mixed_feed=(match_type_selection == "Allow mixed feed"),
                                table_name="Lrg Disc",
                                feed_opens=ld_summary.get('feed_opens', {})
                            )
                            cluster_results['ld_combined'] = ld_matches
                            st.success(f"[OK] Large Discounts matches: {len(ld_matches)} found")
                        else:
                        ...
                        if not recips_prep.empty:
                            st.success(f"[OK] Recips PD prep: {len(recips_prep)} entries")
                            prep_tables['recips_prep'] = recips_prep
                            
                            # Now match pass1 with pass2
                            st.info("⏳ Matching Recips PD entries (checking reciprocal relationships)...")
                            recips_matches = match_cluster_table_entries(
                                ...
                                allow_mixed_feed=(match_type_selection == "Allow mixed feed"),
                                table_name="Recips",
                                feed_opens=recips_summary.get('feed_opens', {})
                            )
                            cluster_results['recips_combined'] = recips_matches
                            st.success(f"[OK] Recips PD matches: {len(recips_matches)} found (verified reciprocals)")
                        else:
                        ...
                        st.warning(f"⏱️ **Timing Summary:** All cluster tables generated in {total_time:.2f}s!")
                ...
                if 'prep_tables' in st.session_state:
                    ...
                    # FOGZ Prep
                    with st.expander("DEBUG: FOGZ Prep Table", expanded=False):
                    ...
                    with st.expander("DEBUG: Large Discounts Prep Table", expanded=False):
                    ...
                    # Recips PD Prep
                    with st.expander("DEBUG: Recips PD Prep Table", expanded=False):
                ...
                if 'cluster_tables' in st.session_state:
                    ...
                    if (('fogz_combined' in cluster_results and len(cluster_results['fogz_combined']) > 0) or
                        ...
                        ('recips_combined' in cluster_results and len(cluster_results['recips_combined']) > 0)):
                        ...
                        try:
                            ...
                            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                                ...
                                if 'fogz_combined' in cluster_results and len(cluster_results['fogz_combined']) > 0:
                                    cluster_results['fogz_combined'].to_excel(writer, sheet_name='FOGZ', index=False)
                                
                                if 'ld_combined' in cluster_results and len(cluster_results['ld_combined']) > 0:
                                    cluster_results['ld_combined'].to_excel(writer, sheet_name='Large Discounts', index=False)
                            ...
                            filename = f"cluster_tables_{timestamp}.xlsx"
                            
                            st.download_button(
                                label="📥 Download All Matched Tables (Excel)",
                                data=output.getvalue(),
                                file_name=filename,
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                key="download_excel"
                            )
                            st.caption("Excel file contains 4 tabs: FOGZ, Large Discounts, Recips PD, and Combined (all tables sorted by Arrival_Output)")
                    ...
                    def highlight_cluster_rows(row):
                        # Check Arrival_Bracket for highlighting
                        # Yellow for [0] (today), Blue for [-1], [-2], [-3] (recent)
                        bracket = str(row.get('Arrival_Bracket', ''))
                        
                        if '[0]' in bracket:
                            return ['background-color: #fff9c4'] * len(row)  # Yellow for today
                        elif any(x in bracket for x in ['[-1]', '[-2]', '[-3]']):
                            return ['background-color: #bbdefb'] * len(row)  # Blue for recent
                        else:
                            return [''] * len(row)  # No highlight for older
                    ...
                    with st.expander(" FOGZ Matched Results", expanded=True):
                        if 'fogz_combined' in cluster_results and len(cluster_results['fogz_combined']) > 0:
                            ...
                            **FOGZ M#s:** {0, +/-1, +/-2, +/-3, +/-5, +/-6} matched with **PwX2_1_0:** {+/-40, +/-41, +/-43, +/-50, +/-55, +/-60, +/-68, +/-77, +/-87, +/-96, +/-103, +/-107, +/-111}
                            ...
                            styled_fogz = cluster_results['fogz_combined'].style.apply(highlight_cluster_rows, axis=1).format({
                                'Arrival_Output': '{:.3f}',
                                'Prox': '{:.3f}'
                            })
                    ...
                    with st.expander(" Large Discounts Matched Results", expanded=False):
                        if 'ld_combined' in cluster_results and len(cluster_results['ld_combined']) > 0:
                            ...
                            **Large Discount M#s:** {+/-36, +/-38, +/-39} matched with **PX2_1_0:** {+/-41, +/-43, +/-50, +/-55, +/-60, +/-68, +/-77, +/-87, +/-96, +/-103, +/-107, +/-111}
                            ...
                            styled_ld = cluster_results['ld_combined'].style.apply(highlight_cluster_rows, axis=1).format({
                                'Arrival_Output': '{:.3f}',
                                'Prox': '{:.3f}'
                            })
                    ...
                    with st.expander(" Recips PD Matched Results", expanded=False):
                        if 'recips_combined' in cluster_results and len(cluster_results['recips_combined']) > 0:
                            ...
                            **Matched with R# mates:**
                            ...
                            styled_recips = cluster_results['recips_combined'].style.apply(highlight_cluster_rows, axis=1).format({
                                'Arrival_Output': '{:.3f}',
                                'Prox': '{:.3f}'
                            })
