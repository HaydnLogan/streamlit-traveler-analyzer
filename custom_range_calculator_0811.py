"""
Custom Range Calculator for Market Data Analysis - FIXED VERSION
Implements sophisticated range calculation with proper input value handling.
"""
...
def get_input_values_batch(small_df, big_df, report_time, start_hour=18):
    ...
    FIXED VERSION - Gets 'open' values from CSV at specific times.
    
    Returns: tuple of (small_input_at_start, big_input_at_start, small_input_at_report, big_input_at_report)
    """
    def get_single_input_value(df, target_time):
        """Helper to get 'open' input value from a single DataFrame at target time"""
...
def get_input_at_time(df, target_time):
    """Get 'open' input value from dataframe at specific time, or closest available time"""
    if df is None or target_time is None or df.empty:
        return None
...
def calculate_wasp_datetime(report_time, weeks_back, start_hour):
    """Calculate WASP datetime (Sunday at start_hour, weeks_back weeks ago)"""
    try:
        report_dt = safe_to_datetime(report_time)
        if pd.isna(report_dt):
            return None
            
        # Find days since last Sunday
        ...
        wasp_datetime = report_dt - timedelta(days=days_since_sunday + 7 * (weeks_back - 1))
        wasp_datetime = wasp_datetime.replace(hour=start_hour, minute=0, second=0, microsecond=0)
        
        return ensure_timezone_naive(wasp_datetime)
    except Exception as e:
        st.error(f"Error calculating WASP datetime: {e}")
        return None

def calculate_macedonia_datetime(report_time, months_back, start_hour):
    """Calculate Macedonia datetime (1st of month at start_hour, months_back months ago)"""
    try:
        report_dt = safe_to_datetime(report_time)
        if pd.isna(report_dt):
            return None
            
        # Start with first day of report month
        macedonia_datetime = report_dt.replace(day=1, hour=start_hour, minute=0, second=0, microsecond=0)
        ...
        return ensure_timezone_naive(macedonia_datetime)
    except Exception as e:
        st.error(f"Error calculating Macedonia datetime: {e}")
        return None
            ...
            try:
                ...
                if raw_m_low <= m_float <= raw_m_high:
                    ...
                    for _, row in matching_rows.iterrows():
                        ...
                        try:
                            ...
                            if pd.isna(arrival_dt):
                                ...
                                if isinstance(arrival_dt, str):
                                    arrival_dt = safe_to_datetime(arrival_dt)
                        ...
                        # FIXED: Use batch input values for performance optimization
...
def process_custom_ranges_advanced(measurement_df, small_df, report_time, custom_ranges, scope_days=20, big_df=None, run_model_g=False):
    ...
    FIXED VERSION with proper WASP/Macedonia handling and input calculations.
    """
    all_valid_entries = []
    processing_summary = []
    ...
    try:
        ...
        # FIXED: Add WASP and Macedonia variants properly
        wasp_variants = ['WASP-12b', 'WASP-12b[1]', 'WASP-12b[2]']
        macedonia_variants = ['Macedonia', 'Macedonia[1]', 'Macedonia[2]']
        ...
        for range_name, range_config in custom_ranges.items():
            ...
            for hlc_df, data_source in data_sources:
                ...
                for origin in origins:
                    # FIXED: Handle WASP origins with proper datetime calculation
                    if (origin.lower().startswith('wasp-12b') or origin.lower() == 'wasp'):
                        
                        # Determine weeks back based on bracket notation
                        weeks_back = 0  # Default to current week
                        if '[1]' in origin:
                            weeks_back = 1  # Last week
                        elif '[2]' in origin:
                            weeks_back = 2  # Two weeks ago
                        
                        # Calculate WASP datetime (Sunday at 18:00, weeks_back weeks ago)
                        wasp_datetime = calculate_wasp_datetime(report_time, weeks_back + 1, 18)
                        if wasp_datetime is None:
                            continue
                            
                        # Get current H/L/C data but use calculated datetime
                    ...
                    # FIXED: Handle Macedonia origins with proper datetime calculation
                    elif (origin.lower().startswith('macedonia')):
                        
                        # Determine months back based on bracket notation
                        months_back = 0  # Default to current month
                        if '[1]' in origin:
                            months_back = 1  # Last month
                        elif '[2]' in origin:
                            months_back = 2  # Two months ago
                        
                        # Calculate Macedonia datetime (1st of month at 18:00, months_back months ago)
                        macedonia_datetime = calculate_macedonia_datetime(report_time, months_back + 1, 18)
                        if macedonia_datetime is None:
                            continue
...
def process_full_range_advanced(measurement_df, small_df, report_time, center, window_radius, scope_days=20, big_df=None, run_model_g=False):
    ...
    try:
        ...
        # FIXED: Include WASP and Macedonia variants properly
        wasp_variants = ['WASP-12b', 'WASP-12b[1]', 'WASP-12b[2]']
        macedonia_variants = ['Macedonia', 'Macedonia[1]', 'Macedonia[2]']
        for variant in wasp_variants + macedonia_variants:
            all_origins.add(variant)
