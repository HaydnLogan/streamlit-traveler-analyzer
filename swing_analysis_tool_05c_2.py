"""
Swing Analysis Tool 05c - Integrated Version
=============================================
Features:
...
st.set_page_config(
    page_title="Swing Analysis Tool 05c",
    page_icon="📊",
    layout="wide"
)
...
def main():
    st.title("📊 Market Swing Analysis Tool 05c_2")
            ...
            with st.spinner("Analyzing daily swings..."):
                ...
                if daily_stats:
                    ...
                    st.download_button(
                        ...
                        file_name=f"swing_analysis_05c_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
            ...
            **Minimum Requirements:**
            ...
            try:
                ...
                if 'recip_report_results' in st.session_state:
                    ...
                    # ========================================
                    st.markdown("---")
                    st.markdown("### 📊 Additional Cluster Tables")
                    ...
                    if 'cluster_tables' in st.session_state:
                        ...
                        # FOGZ Table
                        with st.expander("📋 FOGZ Cluster Table", expanded=True):
                            if len(cluster_results['fogz_combined']) > 0:
                                st.markdown("""
                                **Legend:** 
                                🟡 **Yellow** = Day [0] (Today)  |  
                                🔵 **Blue** = Day [-1] or [-2] (Recent)  |  
                        ...
                        with st.expander("📋 Large Discounts Cluster Table", expanded=False):
                            if len(cluster_results['ld_combined']) > 0:
                                st.markdown("""
                                **Legend:** 
                                🟡 **Yellow** = Day [0] (Today)  |  
                                🔵 **Blue** = Day [-1] or [-2] (Recent)  |  
                                ...
                                **Large Discount M#s:** {±36, ±39} matched with **PX2_1_0:** {±41, ±43, ±50, ±55, ±60, ±68, ±77, ±87, ±96, ±103, ±107, ±111}
                                """)
                        ...
                        # Recips PD Table
                        with st.expander("📋 Recips PD Cluster Table", expanded=False):
                            if len(cluster_results['recips_combined']) > 0:
                                st.markdown("""
                                **Legend:** 
                                🟡 **Yellow** = Day [0] (Today)  |  
                                🔵 **Blue** = Day [-1] or [-2] (Recent)  |  
