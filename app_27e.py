""" v27e - add performance options for fast mode, parallel processing, and minimal display.
Produce one Excel file with 4 measurement tabs. """
...
st.header("🧬 Data Processor + Model A/B/C/G Detector with fast mode. v27e")
...
# Process feeds section (full functionality enabled)
    ...
    try:
        ...
        # Auto-process all 4 measurement tabs for unified Excel export
        if len(available_tabs) > 1:
            st.markdown("**🚀 Auto-Processing All 4 Measurement Tabs for Unified Excel Export**")
            selected_tabs = []
            tab_columns = st.columns(min(4, len(available_tabs)))
            
            # Auto-select first 4 tabs
            for i, tab_name in enumerate(available_tabs[:4]):  # Limit to first 4 tabs
                ...
                with tab_columns[i % 4]:
                    st.success(f"✅ Meas tab {i+1} - {tab_name}")
            
            st.info(f"📊 Processing {len(selected_tabs)} measurement tabs automatically")
        else:
        ...
        if selected_tabs:
            ...
            if parallel_processing:
                st.success("⚡ Parallel Processing Enabled: Processing tabs efficiently")
            ...
            if traveler_reports:
                ...
                with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
                    ...
                    for tab_label, tab_data in traveler_reports.items():
                        if not tab_data.empty:
                            ...
                            # Apply Excel highlighting if not in fast mode
                            if not fast_mode:
                ...
                st.download_button(
                    ...
                    data=excel_buffer,
                    file_name=f"unified_traveler_reports_{timestamp}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
