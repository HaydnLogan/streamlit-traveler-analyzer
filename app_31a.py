# v31 - Updated file uploads (7 files), added asset selector, modified custom ranges to single tab output
...
st.header("🧬 Data Processor + HOD/LOD + Model Detection v31a")
...
# Note: 15m files (small_15m_file and big_15m_file) replace the previous small_feed and big_feed
# These are now used as the primary small and big feeds for processing
small_feed_file = small_15m_file
big_feed_file = big_15m_file
...
# === NORMAL PROCESSING MODE ===
    ...
    try:
        ...
        # Load feeds - using 15m files as primary feeds
        ...
        with upload_info_cols[0]:
            st.metric("Small Feed (15m)", f"{len(small_df)} rows")
        with upload_info_cols[1]:
            st.metric("Big Feed (15m)", f"{len(big_df)} rows")
        with upload_info_cols[2]:
            st.metric("Measurements", f"{len(measurements_df)} rows")
        
        # Display additional file info if other timeframes were uploaded
        additional_files = []
        if small_3m_file: additional_files.append("3m small")
        if small_5m_file: additional_files.append("5m small")
        if big_3m_file: additional_files.append("3m big")
        if big_5m_file: additional_files.append("5m big")
        if additional_files:
            st.info(f"📁 Additional timeframes uploaded: {', '.join(additional_files)} (available for future features)")
        ...
        # ---------- 2) CUSTOM RANGES MODE ----------
            ...
            if final_df_filtered is None or final_df_filtered.empty:
                ...
                # Sort by Group, then Output, then Arrival
                if 'Output' in all_groups_df.columns and 'Arrival' in all_groups_df.columns:
                    all_groups_df = all_groups_df.sort_values(['Group', 'Output', 'Arrival'], ascending=[True, False, True])
                elif 'Output' in all_groups_df.columns:
                    all_groups_df = all_groups_df.sort_values(['Group', 'Output'], ascending=[True, False])
    ...
    st.markdown("""
    - **Small feeds**: 3m, 5m, and 15m timeframes (CSV)
    - **Big feeds**: 3m, 5m, and 15m timeframes (CSV)
    - **Measurement file**: Excel file (XLSX/XLS)
    
    *Note: The 15m timeframes are used as the primary feeds for processing.*
    """)
