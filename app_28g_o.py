# v28g_optimized - Full Range inclusive helper integrated; Full Range runs first.
# Model G now split into multiple files. Batch optimization integrated.
# Master Traveler List fallback removed - only Full Range and Custom Range modes.
...
from a_helpers import (
    clean_timestamp, get_input_at_day_start, apply_excel_highlighting,
...
# Import optimized custom range calculator
...
def render_unified_export(traveler_reports, report_time):
    ...
    st.markdown("---")
    st.markdown("### Excel Download - Master Traveler Groups")
...
st.header("Data Processor + Model A/B/C/G Detector with Batch Optimization v28g_optimized")
...
if run_g_models:
    ...
    run_g10 = st.sidebar.checkbox("   • G.10 (Pair Detection)", value=False, help="Pair detection with neighbor scoring")
    ...
    # Set defaults when G models are disabled
    run_g05_g06 = False
    run_g08 = False
    run_g09 = False
    run_g10 = False
...
if report_type == "Full Range":
    ...
    st.info("**Advanced H/L/C calculation with batch optimization enabled** - Uses sophisticated market data analysis with batch input value calculation for improved performance")
...
# ===========================
    ...
    try:
        st.markdown("### Processing Feed Files with Batch Optimization")
        ...
        if input_value_at_start is not None:
            ...
            st.warning("Could not determine Input @ Day Start - will use manual input or fallback logic")
            
        # Show what the Full Range center will be
        ...
        if use_full_range:
            st.markdown("---")
            st.markdown("### Full Range Processing Mode (Advanced / HLC → raw-M with Batch Optimization)")
            ...
            if result_df is not None and not result_df.empty:
                ...
                st.success("Full Range (Advanced with Batch Optimization) complete.")
            else:
                st.warning("Full Range (Advanced) produced no rows.")

        # ---------- 2) CUSTOM RANGES (advanced with batch optimization) ----------
        elif use_custom_ranges and use_advanced_ranges:
            st.markdown("---")
            st.markdown("### Advanced Custom Range Processing with Batch Optimization")
            ...
            if final_df_filtered is None or final_df_filtered.empty:
                st.warning("No entries found using advanced H/L/C calculation with batch optimization")
                traveler_reports = {}
            else:
                ...
                st.success("Advanced custom range processing with batch optimization complete.")
