...
from custom_range_calculator_1125_9 import (
...
st.set_page_config(
    page_title="Swing Analysis Tool 05e_9",
    page_icon="",
    layout="wide"
)
...
def main():
    st.title(" Market Swing Analysis Tool 05e_9 Two Pass")
            ...
            **Minimum Requirements:**
            ...
            try:
                ...
                with col_filter1:
                    ...
                    st.caption("Process only priority origins. All 9 priority origins selected by default.")
                ...
                if filter_origins:
                    ...
                    with col_e1:
                        include_trinidad = st.checkbox("Trinidad", value=True, key="incl_trinidad")
                    with col_e2:
                        include_tobago = st.checkbox("Tobago", value=True, key="incl_tobago")
                    with col_e3:
                        include_wasp = st.checkbox("WASP-12b", value=True, key="incl_wasp")
                    with col_e4:
                        include_macedonia = st.checkbox("Macedonia", value=True, key="incl_macedonia")
                    
                    st.markdown("**Anchor Origins (Priority):**")
                    col_a1, col_a2, col_a3, col_a4, col_a5 = st.columns(5)
                    with col_a1:
                        include_spain = st.checkbox("Spain", value=True, key="incl_spain")
                    with col_a2:
                        include_saturn = st.checkbox("Saturn", value=True, key="incl_saturn")
                    with col_a3:
                        include_jupiter = st.checkbox("Jupiter", value=True, key="incl_jupiter")
                    with col_a4:
                        include_kepler62 = st.checkbox("Kepler-62", value=True, key="incl_kepler62")
                    with col_a5:
                        include_kepler44 = st.checkbox("Kepler-44", value=True, key="incl_kepler44")
                    
                    st.markdown("**Other Origins (Optional):**")
                    col_o1, col_o2 = st.columns(2)
                    with col_o1:
                        include_fiji = st.checkbox("Fiji", value=False, key="incl_fiji")
                    with col_o2:
                        include_charlie = st.checkbox("Charlie", value=False, key="incl_charlie")
                    
                    # Build allowed origins list
                    allowed_origins = set()
                    # Epic origins
                    if include_trinidad: allowed_origins.add("Trinidad")
                    if include_tobago: allowed_origins.add("Tobago")
                    if include_wasp: allowed_origins.add("WASP-12b")
                    if include_macedonia: allowed_origins.add("Macedonia")
                    # Anchor origins
                    if include_spain: allowed_origins.add("Spain")
                    if include_saturn: allowed_origins.add("Saturn")
                    if include_jupiter: allowed_origins.add("Jupiter")
                    if include_kepler62: allowed_origins.add("Kepler-62")
                    if include_kepler44: allowed_origins.add("Kepler-44")
                    # Other
                    if include_fiji: allowed_origins.add("Fiji")
                    if include_charlie: allowed_origins.add("Charlie")
                    
                    epic_count = sum([include_trinidad, include_tobago, include_wasp, include_macedonia])
                    anchor_count = sum([include_spain, include_saturn, include_jupiter, include_kepler62, include_kepler44])
                    st.info(f"Processing {len(allowed_origins)} origins ({epic_count} Epic + {anchor_count} Anchor + {len(allowed_origins)-epic_count-anchor_count} Other)")
                else:
                ...
                if st.button(" Generate Cluster Tables", key="gen_cluster_tables", type="primary"):
                    with st.spinner("Generating cluster tables with two-pass processing..."):
                        ...
                        if not recips_prep.empty:
                            ...
                            st.success(f"[OK] Recips PD matches: {len(recips_matches)} found")
                        else:
