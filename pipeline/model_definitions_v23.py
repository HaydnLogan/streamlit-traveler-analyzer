"""
Model Definitions v23 - All 24 Trading Models
...
# Union of all five pattern-specific Discount lists above. Safe to combine
# because (a) each list already contains both signs of every magnitude, and
# (b) magnitudes never repeat across the five lists — so the 'disc_opp'
# special-matching rule (same |M#|, opposite sign) can never cross from one
...
def apply_special_matching(model_name, m1, m2):
    ...
    if special == 'lrg_disc_pd':
        ...
        # magnitude but the OPPOSITE sign (e.g. 5 <-> -5, 17 <-> -17).
        # Magnitudes never repeat across D_X0/D_X12/D_XD0/D_XD12/D_XC, so this
        # rule automatically stays within the correct sub-pattern.
        if m1_abs != m2_abs:
            return False
        try:
            m1_pos = float(m1) > 0
